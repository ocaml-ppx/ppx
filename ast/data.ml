...
module Optional = struct
  ...
  module List = struct
    let map list ~f = Option.List.all (List.map list ~f)
  end
...
let of_option x ~f : t = Option (Option.map x ~f)
...
let to_loc t ~f =
  match (t : t) with
  | Loc loc -> Optional.Loc.map loc ~f
  | _ -> None

let to_list t ~f =
  match (t : t) with
  | List x -> Optional.List.map x ~f
  | _ -> None

let to_option t ~f =
  match (t : t) with
  | Option x -> Optional.Option.map x ~f
  | _ -> None

let to_tuple2 t ~f1 ~f2 =
  match (t : t) with
  | Tuple [| x1; x2 |] ->
    Option.bind (f1 x1) ~f:(fun x1 ->
      Option.bind (f2 x2) ~f:(fun x2 ->
        Some (x1, x2)))
  | _ -> None
