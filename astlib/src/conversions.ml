...
let rec loc_of_ast a_of_ast =
  let of_concrete : Stable.V4_07.Loc.Concrete.t -> Astlib_parsetree.loc option =
    fun (Loc { txt; loc }) ->
...
and longident_of_ast =
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Longident.to_concrete x)

and longident_loc_of_ast =
  let of_concrete : Stable.V4_07.Longident_loc.Concrete.t -> Astlib_parsetree.longident_loc option =
    fun (Longident_loc { a }) ->
      Optional.bind ((loc_of_ast longident_of_ast) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Longident_loc.to_concrete x)

and rec_flag_of_ast =
  let of_concrete : Stable.V4_07.Rec_flag.Concrete.t -> Astlib_parsetree.rec_flag option =
    function
    | Nonrecursive -> Some Nonrecursive
    | Recursive -> Some Recursive
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Rec_flag.to_concrete x)

and direction_flag_of_ast =
  let of_concrete : Stable.V4_07.Direction_flag.Concrete.t -> Astlib_parsetree.direction_flag option =
    function
    | Upto -> Some Upto
    | Downto -> Some Downto
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Direction_flag.to_concrete x)

and private_flag_of_ast =
  let of_concrete : Stable.V4_07.Private_flag.Concrete.t -> Astlib_parsetree.private_flag option =
    function
    | Private -> Some Private
    | Public -> Some Public
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Private_flag.to_concrete x)

and mutable_flag_of_ast =
  let of_concrete : Stable.V4_07.Mutable_flag.Concrete.t -> Astlib_parsetree.mutable_flag option =
    function
    | Immutable -> Some Immutable
    | Mutable -> Some Mutable
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Mutable_flag.to_concrete x)

and virtual_flag_of_ast =
  let of_concrete : Stable.V4_07.Virtual_flag.Concrete.t -> Astlib_parsetree.virtual_flag option =
    function
    | Virtual -> Some Virtual
    | Concrete -> Some Concrete
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Virtual_flag.to_concrete x)

and override_flag_of_ast =
  let of_concrete : Stable.V4_07.Override_flag.Concrete.t -> Astlib_parsetree.override_flag option =
    function
    | Override -> Some Override
    | Fresh -> Some Fresh
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Override_flag.to_concrete x)

and closed_flag_of_ast =
  let of_concrete : Stable.V4_07.Closed_flag.Concrete.t -> Astlib_parsetree.closed_flag option =
    function
    | Closed -> Some Closed
    | Open -> Some Open
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Closed_flag.to_concrete x)
...
and label_loc_of_ast =
  let of_concrete : Stable.V4_07.Label_loc.Concrete.t -> Astlib_parsetree.label_loc option =
    fun (Label_loc { a }) ->
      Optional.bind ((loc_of_ast label_of_ast) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Label_loc.to_concrete x)

and string_loc_of_ast =
  let of_concrete : Stable.V4_07.String_loc.Concrete.t -> Astlib_parsetree.string_loc option =
    fun (String_loc { a }) ->
      Optional.bind ((loc_of_ast (fun x -> x)) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.String_loc.to_concrete x)
...
and variance_of_ast =
  let of_concrete : Stable.V4_07.Variance.Concrete.t -> Astlib_parsetree.variance option =
    function
    | Covariant -> Some Covariant
    | Contravariant -> Some Contravariant
    | Invariant -> Some Invariant
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Variance.to_concrete x)
...
and attribute_of_ast =
  let of_concrete : Stable.V4_07.Attribute.Concrete.t -> Astlib_parsetree.attribute option =
    fun (Attribute { a }) ->
      Optional.bind ((fun (x1, x2) -> (string_loc_of_ast x1, payload_of_ast x2)) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Attribute.to_concrete x)

and extension_of_ast =
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Extension.to_concrete x)

and attributes_of_ast =
  let of_concrete : Stable.V4_07.Attributes.Concrete.t -> Astlib_parsetree.attributes option =
    fun (Attributes { a }) ->
      Optional.bind (List.map ~f:(attribute_of_ast) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Attributes.to_concrete x)
  ...
  let of_concrete : Stable.V4_07.Payload.Concrete.t -> Astlib_parsetree.payload option =
    ...
    | PPat (a, b) ->
      Optional.bind (pattern_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.map ~f:(expression_of_ast) b) ~f:(fun b ->
          Some (PPat (a, b))
      ))
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Payload.to_concrete x)

and core_type_of_ast =
...
and core_type_desc_of_ast =
  let of_concrete : Stable.V4_07.Core_type_desc.Concrete.t -> Astlib_parsetree.core_type_desc option =
    ...
    | Ptyp_tuple (a) ->
      Optional.bind (List.map ~f:(core_type_of_ast) a) ~f:(fun a ->
        Some (Ptyp_tuple (a))
      )
    | Ptyp_constr (a, b) ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Optional.bind (List.map ~f:(core_type_of_ast) b) ~f:(fun b ->
          Some (Ptyp_constr (a, b))
      ))
    | Ptyp_object (a, b) ->
      Optional.bind (List.map ~f:(object_field_of_ast) a) ~f:(fun a ->
    ...
    | Ptyp_variant (a, b, c) ->
      Optional.bind (List.map ~f:(row_field_of_ast) a) ~f:(fun a ->
        Optional.bind (closed_flag_of_ast b) ~f:(fun b ->
          Optional.bind (Optional.map ~f:(List.map ~f:(label_of_ast)) c) ~f:(fun c ->
            Some (Ptyp_variant (a, b, c))
      )))
    | Ptyp_poly (a, b) ->
      Optional.bind (List.map ~f:(string_loc_of_ast) a) ~f:(fun a ->
        Optional.bind (core_type_of_ast b) ~f:(fun b ->
          Some (Ptyp_poly (a, b))
      ))
    | Ptyp_package (a) ->
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Core_type_desc.to_concrete x)

and package_type_of_ast =
  let of_concrete : Stable.V4_07.Package_type.Concrete.t -> Astlib_parsetree.package_type option =
    fun (Package_type { a }) ->
      Optional.bind ((fun (x1, x2) -> (longident_loc_of_ast x1, List.map ~f:(package_type_constraint_of_ast) x2)) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Package_type.to_concrete x)

and package_type_constraint_of_ast =
  let of_concrete : Stable.V4_07.Package_type_constraint.Concrete.t -> Astlib_parsetree.package_type_constraint option =
    fun (Package_type_constraint { a }) ->
      Optional.bind ((fun (x1, x2) -> (longident_loc_of_ast x1, core_type_of_ast x2)) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Package_type_constraint.to_concrete x)

and row_field_of_ast =
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Row_field.to_concrete x)

and object_field_of_ast =
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Object_field.to_concrete x)
...
and pattern_desc_of_ast =
  let of_concrete : Stable.V4_07.Pattern_desc.Concrete.t -> Astlib_parsetree.pattern_desc option =
    ...
    | Ppat_tuple (a) ->
      Optional.bind (List.map ~f:(pattern_of_ast) a) ~f:(fun a ->
        Some (Ppat_tuple (a))
      )
    | Ppat_construct (a, b) ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.map ~f:(pattern_of_ast) b) ~f:(fun b ->
    ...
    | Ppat_record (a, b) ->
      Optional.bind (List.map ~f:(record_field_pattern_of_ast) a) ~f:(fun a ->
        Optional.bind (closed_flag_of_ast b) ~f:(fun b ->
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Pattern_desc.to_concrete x)

and record_field_pattern_of_ast =
  let of_concrete : Stable.V4_07.Record_field_pattern.Concrete.t -> Astlib_parsetree.record_field_pattern option =
    fun (Record_field_pattern { a }) ->
      Optional.bind ((fun (x1, x2) -> (longident_loc_of_ast x1, pattern_of_ast x2)) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Record_field_pattern.to_concrete x)

and expression_of_ast =
  ...
  let of_concrete : Stable.V4_07.Expression_desc.Concrete.t -> Astlib_parsetree.expression_desc option =
    ...
    | Pexp_let (a, b, c) ->
      Optional.bind (rec_flag_of_ast a) ~f:(fun a ->
        Optional.bind (List.map ~f:(value_binding_of_ast) b) ~f:(fun b ->
          Optional.bind (expression_of_ast c) ~f:(fun c ->
            Some (Pexp_let (a, b, c))
      )))
    | Pexp_function (a) ->
      Optional.bind (List.map ~f:(case_of_ast) a) ~f:(fun a ->
    ...
    | Pexp_apply (a, b) ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (List.map ~f:(apply_arg_of_ast) b) ~f:(fun b ->
          Some (Pexp_apply (a, b))
      ))
    | Pexp_match (a, b) ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (List.map ~f:(case_of_ast) b) ~f:(fun b ->
          Some (Pexp_match (a, b))
      ))
    | Pexp_try (a, b) ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
    ...
    | Pexp_tuple (a) ->
      Optional.bind (List.map ~f:(expression_of_ast) a) ~f:(fun a ->
        Some (Pexp_tuple (a))
      )
    | Pexp_construct (a, b) ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
    ...
    | Pexp_record (a, b) ->
      Optional.bind (List.map ~f:(record_field_expression_of_ast) a) ~f:(fun a ->
        Optional.bind (Optional.map ~f:(expression_of_ast) b) ~f:(fun b ->
    ...
    | Pexp_ifthenelse (a, b, c) ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (expression_of_ast b) ~f:(fun b ->
          Optional.bind (Optional.map ~f:(expression_of_ast) c) ~f:(fun c ->
    ...
    | Pexp_coerce (a, b, c) ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.map ~f:(core_type_of_ast) b) ~f:(fun b ->
          Optional.bind (core_type_of_ast c) ~f:(fun c ->
    ...
    | Pexp_override (a) ->
      Optional.bind (List.map ~f:(override_expression_of_ast) a) ~f:(fun a ->
...
and override_expression_of_ast =
  let of_concrete : Stable.V4_07.Override_expression.Concrete.t -> Astlib_parsetree.override_expression option =
    fun (Override_expression { a }) ->
      Optional.bind ((fun (x1, x2) -> (label_loc_of_ast x1, expression_of_ast x2)) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Override_expression.to_concrete x)

and record_field_expression_of_ast =
  let of_concrete : Stable.V4_07.Record_field_expression.Concrete.t -> Astlib_parsetree.record_field_expression option =
    fun (Record_field_expression { a }) ->
      Optional.bind ((fun (x1, x2) -> (longident_loc_of_ast x1, expression_of_ast x2)) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Record_field_expression.to_concrete x)

and apply_arg_of_ast =
  let of_concrete : Stable.V4_07.Apply_arg.Concrete.t -> Astlib_parsetree.apply_arg option =
    fun (Apply_arg { a }) ->
      Optional.bind ((fun (x1, x2) -> (arg_label_of_ast x1, expression_of_ast x2)) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Apply_arg.to_concrete x)
  ...
  let of_concrete : Stable.V4_07.Case.Concrete.t -> Astlib_parsetree.case option =
    fun (Case { pc_lhs; pc_guard; pc_rhs }) ->
      Optional.bind (pattern_of_ast pc_lhs) ~f:(fun pc_lhs ->
        Optional.bind (Optional.map ~f:(expression_of_ast) pc_guard) ~f:(fun pc_guard ->
          Optional.bind (expression_of_ast pc_rhs) ~f:(fun pc_rhs ->
            Some ({ pc_lhs; pc_guard; pc_rhs } : Astlib_parsetree.case)
      )))
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Case.to_concrete x)

and value_description_of_ast =
  ...
  let of_concrete : Stable.V4_07.Type_declaration.Concrete.t -> Astlib_parsetree.type_declaration option =
    fun (Type_declaration { ptype_name; ptype_params; ptype_cstrs; ptype_kind; ptype_private; ptype_manifest; ptype_attributes; ptype_loc }) ->
      Optional.bind (string_loc_of_ast ptype_name) ~f:(fun ptype_name ->
        Optional.bind (List.map ~f:(type_param_of_ast) ptype_params) ~f:(fun ptype_params ->
          Optional.bind (List.map ~f:(type_constraint_of_ast) ptype_cstrs) ~f:(fun ptype_cstrs ->
...
and type_param_of_ast =
  let of_concrete : Stable.V4_07.Type_param.Concrete.t -> Astlib_parsetree.type_param option =
    fun (Type_param { a }) ->
      Optional.bind ((fun (x1, x2) -> (core_type_of_ast x1, variance_of_ast x2)) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Type_param.to_concrete x)

and type_constraint_of_ast =
  let of_concrete : Stable.V4_07.Type_constraint.Concrete.t -> Astlib_parsetree.type_constraint option =
    fun (Type_constraint { a }) ->
      Optional.bind ((fun (x1, x2, x3) -> (core_type_of_ast x1, core_type_of_ast x2, x3)) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Type_constraint.to_concrete x)
...
and constructor_declaration_of_ast =
...
and constructor_arguments_of_ast =
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Constructor_arguments.to_concrete x)

and type_extension_of_ast =
  let of_concrete : Stable.V4_07.Type_extension.Concrete.t -> Astlib_parsetree.type_extension option =
    fun (Type_extension { ptyext_path; ptyext_params; ptyext_constructors; ptyext_private; ptyext_attributes }) ->
      Optional.bind (longident_loc_of_ast ptyext_path) ~f:(fun ptyext_path ->
        Optional.bind (List.map ~f:(type_param_of_ast) ptyext_params) ~f:(fun ptyext_params ->
          Optional.bind (List.map ~f:(extension_constructor_of_ast) ptyext_constructors) ~f:(fun ptyext_constructors ->
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Type_extension.to_concrete x)

and extension_constructor_of_ast =
...
and extension_constructor_kind_of_ast =
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Extension_constructor_kind.to_concrete x)

and class_type_of_ast =
...
and class_type_desc_of_ast =
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Class_type_desc.to_concrete x)

and class_signature_of_ast =
  let of_concrete : Stable.V4_07.Class_signature.Concrete.t -> Astlib_parsetree.class_signature option =
    fun (Class_signature { pcsig_self; pcsig_fields }) ->
      Optional.bind (core_type_of_ast pcsig_self) ~f:(fun pcsig_self ->
        Optional.bind (List.map ~f:(class_type_field_of_ast) pcsig_fields) ~f:(fun pcsig_fields ->
          Some ({ pcsig_self; pcsig_fields } : Astlib_parsetree.class_signature)
      ))
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Class_signature.to_concrete x)

and class_type_field_of_ast =
...
and class_type_field_desc_of_ast =
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Class_type_field_desc.to_concrete x)

and class_type_value_desc_of_ast =
  let of_concrete : Stable.V4_07.Class_type_value_desc.Concrete.t -> Astlib_parsetree.class_type_value_desc option =
    fun (Class_type_value_desc { a }) ->
      Optional.bind ((fun (x1, x2, x3, x4) -> (label_loc_of_ast x1, mutable_flag_of_ast x2, virtual_flag_of_ast x3, core_type_of_ast x4)) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Class_type_value_desc.to_concrete x)

and class_type_method_desc_of_ast =
  let of_concrete : Stable.V4_07.Class_type_method_desc.Concrete.t -> Astlib_parsetree.class_type_method_desc option =
    fun (Class_type_method_desc { a }) ->
      Optional.bind ((fun (x1, x2, x3, x4) -> (label_loc_of_ast x1, private_flag_of_ast x2, virtual_flag_of_ast x3, core_type_of_ast x4)) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Class_type_method_desc.to_concrete x)

and class_type_constraint_of_ast =
  let of_concrete : Stable.V4_07.Class_type_constraint.Concrete.t -> Astlib_parsetree.class_type_constraint option =
    fun (Class_type_constraint { a }) ->
      Optional.bind ((fun (x1, x2) -> (core_type_of_ast x1, core_type_of_ast x2)) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Class_type_constraint.to_concrete x)

and class_infos_of_ast a_of_ast =
  let of_concrete : Stable.V4_07.Class_infos.Concrete.t -> Astlib_parsetree.class_infos option =
    fun (Class_infos { pci_virt; pci_params; pci_name; pci_expr; pci_loc; pci_attributes }) ->
      Optional.bind (virtual_flag_of_ast pci_virt) ~f:(fun pci_virt ->
        Optional.bind (List.map ~f:(type_param_of_ast) pci_params) ~f:(fun pci_params ->
...
and class_description_of_ast =
  let of_concrete : Stable.V4_07.Class_description.Concrete.t -> Astlib_parsetree.class_description option =
    fun (Class_description { a }) ->
      Optional.bind ((class_infos_of_ast class_type_of_ast) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Class_description.to_concrete x)

and class_type_declaration_of_ast =
  let of_concrete : Stable.V4_07.Class_type_declaration.Concrete.t -> Astlib_parsetree.class_type_declaration option =
    fun (Class_type_declaration { a }) ->
      Optional.bind ((class_infos_of_ast class_type_of_ast) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Class_type_declaration.to_concrete x)

and class_expr_of_ast =
...
and class_expr_desc_of_ast =
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Class_expr_desc.to_concrete x)

and class_structure_of_ast =
  let of_concrete : Stable.V4_07.Class_structure.Concrete.t -> Astlib_parsetree.class_structure option =
    fun (Class_structure { pcstr_self; pcstr_fields }) ->
      Optional.bind (pattern_of_ast pcstr_self) ~f:(fun pcstr_self ->
        Optional.bind (List.map ~f:(class_field_of_ast) pcstr_fields) ~f:(fun pcstr_fields ->
          Some ({ pcstr_self; pcstr_fields } : Astlib_parsetree.class_structure)
      ))
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Class_structure.to_concrete x)

and class_field_of_ast =
...
and class_field_desc_of_ast =
    ...
    | Pcf_inherit (a, b, c) ->
      Optional.bind (override_flag_of_ast a) ~f:(fun a ->
        Optional.bind (class_expr_of_ast b) ~f:(fun b ->
          Optional.bind (Optional.map ~f:(string_loc_of_ast) c) ~f:(fun c ->
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Class_field_desc.to_concrete x)

and class_value_desc_of_ast =
  let of_concrete : Stable.V4_07.Class_value_desc.Concrete.t -> Astlib_parsetree.class_value_desc option =
    fun (Class_value_desc { a }) ->
      Optional.bind ((fun (x1, x2, x3) -> (label_loc_of_ast x1, mutable_flag_of_ast x2, class_field_kind_of_ast x3)) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Class_value_desc.to_concrete x)

and class_method_desc_of_ast =
  let of_concrete : Stable.V4_07.Class_method_desc.Concrete.t -> Astlib_parsetree.class_method_desc option =
    fun (Class_method_desc { a }) ->
      Optional.bind ((fun (x1, x2, x3) -> (label_loc_of_ast x1, private_flag_of_ast x2, class_field_kind_of_ast x3)) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Class_method_desc.to_concrete x)

and class_field_kind_of_ast =
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Class_field_kind.to_concrete x)

and class_declaration_of_ast =
  let of_concrete : Stable.V4_07.Class_declaration.Concrete.t -> Astlib_parsetree.class_declaration option =
    fun (Class_declaration { a }) ->
      Optional.bind ((class_infos_of_ast class_expr_of_ast) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Class_declaration.to_concrete x)

and module_type_of_ast =
...
and module_type_desc_of_ast =
  let of_concrete : Stable.V4_07.Module_type_desc.Concrete.t -> Astlib_parsetree.module_type_desc option =
    ...
    | Pmty_functor (a, b, c) ->
      Optional.bind (string_loc_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.map ~f:(module_type_of_ast) b) ~f:(fun b ->
          Optional.bind (module_type_of_ast c) ~f:(fun c ->
            Some (Pmty_functor (a, b, c))
      )))
    | Pmty_with (a, b) ->
      Optional.bind (module_type_of_ast a) ~f:(fun a ->
        Optional.bind (List.map ~f:(with_constraint_of_ast) b) ~f:(fun b ->
          Some (Pmty_with (a, b))
      ))
    | Pmty_typeof (a) ->
      Optional.bind (module_expr_of_ast a) ~f:(fun a ->
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Module_type_desc.to_concrete x)

and signature_of_ast =
  let of_concrete : Stable.V4_07.Signature.Concrete.t -> Astlib_parsetree.signature option =
    fun (Signature { a }) ->
      Optional.bind (List.map ~f:(signature_item_of_ast) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Signature.to_concrete x)

and signature_item_of_ast =
...
and signature_item_desc_of_ast =
  let of_concrete : Stable.V4_07.Signature_item_desc.Concrete.t -> Astlib_parsetree.signature_item_desc option =
    ...
    | Psig_type (a, b) ->
      Optional.bind (rec_flag_of_ast a) ~f:(fun a ->
        Optional.bind (List.map ~f:(type_declaration_of_ast) b) ~f:(fun b ->
          Some (Psig_type (a, b))
      ))
    | Psig_typext (a) ->
    ...
    | Psig_recmodule (a) ->
      Optional.bind (List.map ~f:(module_declaration_of_ast) a) ~f:(fun a ->
    ...
    | Psig_class (a) ->
      Optional.bind (List.map ~f:(class_description_of_ast) a) ~f:(fun a ->
        Some (Psig_class (a))
      )
    | Psig_class_type (a) ->
      Optional.bind (List.map ~f:(class_type_declaration_of_ast) a) ~f:(fun a ->
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Signature_item_desc.to_concrete x)

and module_declaration_of_ast =
...
and module_type_declaration_of_ast =
...
and include_infos_of_ast a_of_ast =
  let of_concrete : Stable.V4_07.Include_infos.Concrete.t -> Astlib_parsetree.include_infos option =
    fun (Include_infos { pincl_mod; pincl_loc; pincl_attributes }) ->
...
and include_description_of_ast =
  let of_concrete : Stable.V4_07.Include_description.Concrete.t -> Astlib_parsetree.include_description option =
    fun (Include_description { a }) ->
      Optional.bind ((include_infos_of_ast module_type_of_ast) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Include_description.to_concrete x)

and include_declaration_of_ast =
  let of_concrete : Stable.V4_07.Include_declaration.Concrete.t -> Astlib_parsetree.include_declaration option =
    fun (Include_declaration { a }) ->
      Optional.bind ((include_infos_of_ast module_expr_of_ast) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Include_declaration.to_concrete x)

and with_constraint_of_ast =
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.With_constraint.to_concrete x)

and module_expr_of_ast =
...
and module_expr_desc_of_ast =
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Module_expr_desc.to_concrete x)

and structure_of_ast =
  let of_concrete : Stable.V4_07.Structure.Concrete.t -> Astlib_parsetree.structure option =
    fun (Structure { a }) ->
      Optional.bind (List.map ~f:(structure_item_of_ast) a) ~f:(fun a ->
        Some a
      )
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Structure.to_concrete x)

and structure_item_of_ast =
...
and structure_item_desc_of_ast =
  let of_concrete : Stable.V4_07.Structure_item_desc.Concrete.t -> Astlib_parsetree.structure_item_desc option =
    ...
    | Pstr_recmodule (a) ->
      Optional.bind (List.map ~f:(module_binding_of_ast) a) ~f:(fun a ->
    ...
    | Pstr_class (a) ->
      Optional.bind (List.map ~f:(class_declaration_of_ast) a) ~f:(fun a ->
        Some (Pstr_class (a))
      )
    | Pstr_class_type (a) ->
      Optional.bind (List.map ~f:(class_type_declaration_of_ast) a) ~f:(fun a ->
  ...
  in
  fun x -> Optional.bind ~f:of_concrete (Stable.V4_07.Structure_item_desc.to_concrete x)

and value_binding_of_ast =
...
let rec loc_to_ast a_to_ast : Astlib_parsetree.loc -> Stable.V4_07.Loc.t =
  fun { txt; loc } ->
    let txt = a_to_ast txt in
    Stable.V4_07.Loc.of_concrete { txt; loc }

and longident_to_ast : Astlib_parsetree.longident -> Stable.V4_07.Longident.t =
  function
  | Lident (a) ->
...
and longident_loc_to_ast : Astlib_parsetree.longident_loc -> Stable.V4_07.Longident_loc.t =
  fun x ->
    let x = (loc_to_ast longident_to_ast) x in
    Stable.V4_07.Longident_loc.of_concrete x

and rec_flag_to_ast : Astlib_parsetree.rec_flag -> Stable.V4_07.Rec_flag.t =
  function
  | Nonrecursive -> Stable.V4_07.Rec_flag.of_concrete Nonrecursive
  | Recursive -> Stable.V4_07.Rec_flag.of_concrete Recursive

and direction_flag_to_ast : Astlib_parsetree.direction_flag -> Stable.V4_07.Direction_flag.t =
  function
  | Upto -> Stable.V4_07.Direction_flag.of_concrete Upto
  | Downto -> Stable.V4_07.Direction_flag.of_concrete Downto

and private_flag_to_ast : Astlib_parsetree.private_flag -> Stable.V4_07.Private_flag.t =
  function
  | Private -> Stable.V4_07.Private_flag.of_concrete Private
  | Public -> Stable.V4_07.Private_flag.of_concrete Public

and mutable_flag_to_ast : Astlib_parsetree.mutable_flag -> Stable.V4_07.Mutable_flag.t =
  function
  | Immutable -> Stable.V4_07.Mutable_flag.of_concrete Immutable
  | Mutable -> Stable.V4_07.Mutable_flag.of_concrete Mutable

and virtual_flag_to_ast : Astlib_parsetree.virtual_flag -> Stable.V4_07.Virtual_flag.t =
  function
  | Virtual -> Stable.V4_07.Virtual_flag.of_concrete Virtual
  | Concrete -> Stable.V4_07.Virtual_flag.of_concrete Concrete

and override_flag_to_ast : Astlib_parsetree.override_flag -> Stable.V4_07.Override_flag.t =
  function
  | Override -> Stable.V4_07.Override_flag.of_concrete Override
  | Fresh -> Stable.V4_07.Override_flag.of_concrete Fresh

and closed_flag_to_ast : Astlib_parsetree.closed_flag -> Stable.V4_07.Closed_flag.t =
  function
  | Closed -> Stable.V4_07.Closed_flag.of_concrete Closed
  | Open -> Stable.V4_07.Closed_flag.of_concrete Open

and label_to_ast : Astlib_parsetree.label -> Stable.V4_07.Label.t =
  fun x ->
    Stable.V4_07.Label.of_concrete x

and label_loc_to_ast : Astlib_parsetree.label_loc -> Stable.V4_07.Label_loc.t =
  fun x ->
    let x = (loc_to_ast label_to_ast) x in
    Stable.V4_07.Label_loc.of_concrete x

and string_loc_to_ast : Astlib_parsetree.string_loc -> Stable.V4_07.String_loc.t =
  fun x ->
    let x = (loc_to_ast (fun x -> x)) x in
    Stable.V4_07.String_loc.of_concrete x
...
and variance_to_ast : Astlib_parsetree.variance -> Stable.V4_07.Variance.t =
  function
  | Covariant -> Stable.V4_07.Variance.of_concrete Covariant
  | Contravariant -> Stable.V4_07.Variance.of_concrete Contravariant
  | Invariant -> Stable.V4_07.Variance.of_concrete Invariant
...
and attribute_to_ast : Astlib_parsetree.attribute -> Stable.V4_07.Attribute.t =
  fun x ->
    let x = (fun (x1, x2) -> (string_loc_to_ast x1, payload_to_ast x2)) x in
    Stable.V4_07.Attribute.of_concrete x

and extension_to_ast : Astlib_parsetree.extension -> Stable.V4_07.Extension.t =
  fun x ->
    let x = (fun (x1, x2) -> (string_loc_to_ast x1, payload_to_ast x2)) x in
    Stable.V4_07.Extension.of_concrete x

and attributes_to_ast : Astlib_parsetree.attributes -> Stable.V4_07.Attributes.t =
  fun x ->
    let x = List.map ~f:(attribute_to_ast) x in
    Stable.V4_07.Attributes.of_concrete x
...
and core_type_to_ast : Astlib_parsetree.core_type -> Stable.V4_07.Core_type.t =
  fun { ptyp_desc; ptyp_loc; ptyp_attributes } ->
    ...
    Stable.V4_07.Core_type.of_concrete { ptyp_desc; ptyp_loc; ptyp_attributes }
...
and package_type_to_ast : Astlib_parsetree.package_type -> Stable.V4_07.Package_type.t =
  fun x ->
    let x = (fun (x1, x2) -> (longident_loc_to_ast x1, List.map ~f:(package_type_constraint_to_ast) x2)) x in
    Stable.V4_07.Package_type.of_concrete x

and package_type_constraint_to_ast : Astlib_parsetree.package_type_constraint -> Stable.V4_07.Package_type_constraint.t =
  fun x ->
    let x = (fun (x1, x2) -> (longident_loc_to_ast x1, core_type_to_ast x2)) x in
    Stable.V4_07.Package_type_constraint.of_concrete x
...
and pattern_to_ast : Astlib_parsetree.pattern -> Stable.V4_07.Pattern.t =
  fun { ppat_desc; ppat_loc; ppat_attributes } ->
    ...
    Stable.V4_07.Pattern.of_concrete { ppat_desc; ppat_loc; ppat_attributes }
...
and record_field_pattern_to_ast : Astlib_parsetree.record_field_pattern -> Stable.V4_07.Record_field_pattern.t =
  fun x ->
    let x = (fun (x1, x2) -> (longident_loc_to_ast x1, pattern_to_ast x2)) x in
    Stable.V4_07.Record_field_pattern.of_concrete x
...
and expression_to_ast : Astlib_parsetree.expression -> Stable.V4_07.Expression.t =
  fun { pexp_desc; pexp_loc; pexp_attributes } ->
    ...
    Stable.V4_07.Expression.of_concrete { pexp_desc; pexp_loc; pexp_attributes }
...
and override_expression_to_ast : Astlib_parsetree.override_expression -> Stable.V4_07.Override_expression.t =
  fun x ->
    let x = (fun (x1, x2) -> (label_loc_to_ast x1, expression_to_ast x2)) x in
    Stable.V4_07.Override_expression.of_concrete x

and record_field_expression_to_ast : Astlib_parsetree.record_field_expression -> Stable.V4_07.Record_field_expression.t =
  fun x ->
    let x = (fun (x1, x2) -> (longident_loc_to_ast x1, expression_to_ast x2)) x in
    Stable.V4_07.Record_field_expression.of_concrete x

and apply_arg_to_ast : Astlib_parsetree.apply_arg -> Stable.V4_07.Apply_arg.t =
  fun x ->
    let x = (fun (x1, x2) -> (arg_label_to_ast x1, expression_to_ast x2)) x in
    Stable.V4_07.Apply_arg.of_concrete x
...
and case_to_ast : Astlib_parsetree.case -> Stable.V4_07.Case.t =
  fun { pc_lhs; pc_guard; pc_rhs } ->
    ...
    Stable.V4_07.Case.of_concrete { pc_lhs; pc_guard; pc_rhs }
...
and value_description_to_ast : Astlib_parsetree.value_description -> Stable.V4_07.Value_description.t =
  fun { pval_name; pval_type; pval_prim; pval_attributes; pval_loc } ->
    ...
    Stable.V4_07.Value_description.of_concrete { pval_name; pval_type; pval_prim; pval_attributes; pval_loc }
...
and type_declaration_to_ast : Astlib_parsetree.type_declaration -> Stable.V4_07.Type_declaration.t =
  fun { ptype_name; ptype_params; ptype_cstrs; ptype_kind; ptype_private; ptype_manifest; ptype_attributes; ptype_loc } ->
    ...
    Stable.V4_07.Type_declaration.of_concrete { ptype_name; ptype_params; ptype_cstrs; ptype_kind; ptype_private; ptype_manifest; ptype_attributes; ptype_loc }

and type_param_to_ast : Astlib_parsetree.type_param -> Stable.V4_07.Type_param.t =
  fun x ->
    let x = (fun (x1, x2) -> (core_type_to_ast x1, variance_to_ast x2)) x in
    Stable.V4_07.Type_param.of_concrete x

and type_constraint_to_ast : Astlib_parsetree.type_constraint -> Stable.V4_07.Type_constraint.t =
  fun x ->
    let x = (fun (x1, x2, x3) -> (core_type_to_ast x1, core_type_to_ast x2, x3)) x in
    Stable.V4_07.Type_constraint.of_concrete x
...
and label_declaration_to_ast : Astlib_parsetree.label_declaration -> Stable.V4_07.Label_declaration.t =
  fun { pld_name; pld_mutable; pld_type; pld_loc; pld_attributes } ->
    ...
    Stable.V4_07.Label_declaration.of_concrete { pld_name; pld_mutable; pld_type; pld_loc; pld_attributes }
...
and constructor_declaration_to_ast : Astlib_parsetree.constructor_declaration -> Stable.V4_07.Constructor_declaration.t =
  fun { pcd_name; pcd_args; pcd_res; pcd_loc; pcd_attributes } ->
    ...
    Stable.V4_07.Constructor_declaration.of_concrete { pcd_name; pcd_args; pcd_res; pcd_loc; pcd_attributes }
...
and type_extension_to_ast : Astlib_parsetree.type_extension -> Stable.V4_07.Type_extension.t =
  fun { ptyext_path; ptyext_params; ptyext_constructors; ptyext_private; ptyext_attributes } ->
    ...
    Stable.V4_07.Type_extension.of_concrete { ptyext_path; ptyext_params; ptyext_constructors; ptyext_private; ptyext_attributes }
...
and extension_constructor_to_ast : Astlib_parsetree.extension_constructor -> Stable.V4_07.Extension_constructor.t =
  fun { pext_name; pext_kind; pext_loc; pext_attributes } ->
    ...
    Stable.V4_07.Extension_constructor.of_concrete { pext_name; pext_kind; pext_loc; pext_attributes }
...
and class_type_to_ast : Astlib_parsetree.class_type -> Stable.V4_07.Class_type.t =
  fun { pcty_desc; pcty_loc; pcty_attributes } ->
    ...
    Stable.V4_07.Class_type.of_concrete { pcty_desc; pcty_loc; pcty_attributes }
...
and class_signature_to_ast : Astlib_parsetree.class_signature -> Stable.V4_07.Class_signature.t =
  fun { pcsig_self; pcsig_fields } ->
    ...
    Stable.V4_07.Class_signature.of_concrete { pcsig_self; pcsig_fields }
...
and class_type_field_to_ast : Astlib_parsetree.class_type_field -> Stable.V4_07.Class_type_field.t =
  fun { pctf_desc; pctf_loc; pctf_attributes } ->
    ...
    Stable.V4_07.Class_type_field.of_concrete { pctf_desc; pctf_loc; pctf_attributes }
...
and class_type_value_desc_to_ast : Astlib_parsetree.class_type_value_desc -> Stable.V4_07.Class_type_value_desc.t =
  fun x ->
    let x = (fun (x1, x2, x3, x4) -> (label_loc_to_ast x1, mutable_flag_to_ast x2, virtual_flag_to_ast x3, core_type_to_ast x4)) x in
    Stable.V4_07.Class_type_value_desc.of_concrete x

and class_type_method_desc_to_ast : Astlib_parsetree.class_type_method_desc -> Stable.V4_07.Class_type_method_desc.t =
  fun x ->
    let x = (fun (x1, x2, x3, x4) -> (label_loc_to_ast x1, private_flag_to_ast x2, virtual_flag_to_ast x3, core_type_to_ast x4)) x in
    Stable.V4_07.Class_type_method_desc.of_concrete x

and class_type_constraint_to_ast : Astlib_parsetree.class_type_constraint -> Stable.V4_07.Class_type_constraint.t =
  fun x ->
    let x = (fun (x1, x2) -> (core_type_to_ast x1, core_type_to_ast x2)) x in
    Stable.V4_07.Class_type_constraint.of_concrete x

and class_infos_to_ast a_to_ast : Astlib_parsetree.class_infos -> Stable.V4_07.Class_infos.t =
  fun { pci_virt; pci_params; pci_name; pci_expr; pci_loc; pci_attributes } ->
    ...
    let pci_expr = a_to_ast pci_expr in
    let pci_attributes = attributes_to_ast pci_attributes in
    Stable.V4_07.Class_infos.of_concrete { pci_virt; pci_params; pci_name; pci_expr; pci_loc; pci_attributes }

and class_description_to_ast : Astlib_parsetree.class_description -> Stable.V4_07.Class_description.t =
  fun x ->
    let x = (class_infos_to_ast class_type_to_ast) x in
    Stable.V4_07.Class_description.of_concrete x

and class_type_declaration_to_ast : Astlib_parsetree.class_type_declaration -> Stable.V4_07.Class_type_declaration.t =
  fun x ->
    let x = (class_infos_to_ast class_type_to_ast) x in
    Stable.V4_07.Class_type_declaration.of_concrete x
...
and class_expr_to_ast : Astlib_parsetree.class_expr -> Stable.V4_07.Class_expr.t =
  fun { pcl_desc; pcl_loc; pcl_attributes } ->
    ...
    Stable.V4_07.Class_expr.of_concrete { pcl_desc; pcl_loc; pcl_attributes }
...
and class_structure_to_ast : Astlib_parsetree.class_structure -> Stable.V4_07.Class_structure.t =
  fun { pcstr_self; pcstr_fields } ->
    ...
    Stable.V4_07.Class_structure.of_concrete { pcstr_self; pcstr_fields }
...
and class_field_to_ast : Astlib_parsetree.class_field -> Stable.V4_07.Class_field.t =
  fun { pcf_desc; pcf_loc; pcf_attributes } ->
    ...
    Stable.V4_07.Class_field.of_concrete { pcf_desc; pcf_loc; pcf_attributes }
...
and class_value_desc_to_ast : Astlib_parsetree.class_value_desc -> Stable.V4_07.Class_value_desc.t =
  fun x ->
    let x = (fun (x1, x2, x3) -> (label_loc_to_ast x1, mutable_flag_to_ast x2, class_field_kind_to_ast x3)) x in
    Stable.V4_07.Class_value_desc.of_concrete x

and class_method_desc_to_ast : Astlib_parsetree.class_method_desc -> Stable.V4_07.Class_method_desc.t =
  fun x ->
    let x = (fun (x1, x2, x3) -> (label_loc_to_ast x1, private_flag_to_ast x2, class_field_kind_to_ast x3)) x in
    Stable.V4_07.Class_method_desc.of_concrete x
...
and class_declaration_to_ast : Astlib_parsetree.class_declaration -> Stable.V4_07.Class_declaration.t =
  fun x ->
    let x = (class_infos_to_ast class_expr_to_ast) x in
    Stable.V4_07.Class_declaration.of_concrete x
...
and module_type_to_ast : Astlib_parsetree.module_type -> Stable.V4_07.Module_type.t =
  fun { pmty_desc; pmty_loc; pmty_attributes } ->
    ...
    Stable.V4_07.Module_type.of_concrete { pmty_desc; pmty_loc; pmty_attributes }
...
and signature_to_ast : Astlib_parsetree.signature -> Stable.V4_07.Signature.t =
  fun x ->
    let x = List.map ~f:(signature_item_to_ast) x in
    Stable.V4_07.Signature.of_concrete x
...
and signature_item_to_ast : Astlib_parsetree.signature_item -> Stable.V4_07.Signature_item.t =
  fun { psig_desc; psig_loc } ->
    ...
    Stable.V4_07.Signature_item.of_concrete { psig_desc; psig_loc }
...
and module_declaration_to_ast : Astlib_parsetree.module_declaration -> Stable.V4_07.Module_declaration.t =
  fun { pmd_name; pmd_type; pmd_attributes; pmd_loc } ->
    ...
    Stable.V4_07.Module_declaration.of_concrete { pmd_name; pmd_type; pmd_attributes; pmd_loc }
...
and module_type_declaration_to_ast : Astlib_parsetree.module_type_declaration -> Stable.V4_07.Module_type_declaration.t =
  fun { pmtd_name; pmtd_type; pmtd_attributes; pmtd_loc } ->
    ...
    Stable.V4_07.Module_type_declaration.of_concrete { pmtd_name; pmtd_type; pmtd_attributes; pmtd_loc }
...
and open_description_to_ast : Astlib_parsetree.open_description -> Stable.V4_07.Open_description.t =
  fun { popen_lid; popen_override; popen_loc; popen_attributes } ->
    ...
    Stable.V4_07.Open_description.of_concrete { popen_lid; popen_override; popen_loc; popen_attributes }

and include_infos_to_ast a_to_ast : Astlib_parsetree.include_infos -> Stable.V4_07.Include_infos.t =
  fun { pincl_mod; pincl_loc; pincl_attributes } ->
    let pincl_mod = a_to_ast pincl_mod in
    let pincl_attributes = attributes_to_ast pincl_attributes in
    Stable.V4_07.Include_infos.of_concrete { pincl_mod; pincl_loc; pincl_attributes }

and include_description_to_ast : Astlib_parsetree.include_description -> Stable.V4_07.Include_description.t =
  fun x ->
    let x = (include_infos_to_ast module_type_to_ast) x in
    Stable.V4_07.Include_description.of_concrete x

and include_declaration_to_ast : Astlib_parsetree.include_declaration -> Stable.V4_07.Include_declaration.t =
  fun x ->
    let x = (include_infos_to_ast module_expr_to_ast) x in
    Stable.V4_07.Include_declaration.of_concrete x
...
and module_expr_to_ast : Astlib_parsetree.module_expr -> Stable.V4_07.Module_expr.t =
  fun { pmod_desc; pmod_loc; pmod_attributes } ->
    ...
    Stable.V4_07.Module_expr.of_concrete { pmod_desc; pmod_loc; pmod_attributes }
...
and structure_to_ast : Astlib_parsetree.structure -> Stable.V4_07.Structure.t =
  fun x ->
    let x = List.map ~f:(structure_item_to_ast) x in
    Stable.V4_07.Structure.of_concrete x
...
and structure_item_to_ast : Astlib_parsetree.structure_item -> Stable.V4_07.Structure_item.t =
  fun { pstr_desc; pstr_loc } ->
    ...
    Stable.V4_07.Structure_item.of_concrete { pstr_desc; pstr_loc }
...
and value_binding_to_ast : Astlib_parsetree.value_binding -> Stable.V4_07.Value_binding.t =
  fun { pvb_pat; pvb_expr; pvb_attributes; pvb_loc } ->
    ...
    Stable.V4_07.Value_binding.of_concrete { pvb_pat; pvb_expr; pvb_attributes; pvb_loc }
...
and module_binding_to_ast : Astlib_parsetree.module_binding -> Stable.V4_07.Module_binding.t =
  fun { pmb_name; pmb_expr; pmb_attributes; pmb_loc } ->
    ...
    Stable.V4_07.Module_binding.of_concrete { pmb_name; pmb_expr; pmb_attributes; pmb_loc }
