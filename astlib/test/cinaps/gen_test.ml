...
let version_index_exn =
  let rec loop ~index ~version alist =
    ...
    | [] ->
      Base.raise_s
        (Sexp.message
           "Astlib_src_cinaps.version_index_exn: no such version"
    ...
    Print.indented (fun () ->
      ...
      Print.indented (fun () ->
        ...
        Print.indented (fun () ->
          ...
          Print.format "in";
          Print.format
            "if not (Ast.equal (Astlib.%s.%s.to_ast x) (Astlib.%s.%s.to_ast y))"
            version
            kind.kind_name
            version
            kind.kind_name;
          ...
          Print.indented (fun () ->
            Print.format
              "let x = Ast.sexp_of_t (Astlib.%s.%s.to_ast x) in"
              version
              kind.kind_name;
            Print.format
              "let y = Ast.sexp_of_t (Astlib.%s.%s.to_ast y) in"
              version
              kind.kind_name;
            Print.format
...
let print_astlib_test_ml () =
  ...
  in
  let entry_points =
    List.map Astlib_parsetree_types.entry_points ~f:String.capitalize
  in
  Print.newline ();
