...
let parsetree decl_name = "Parsetree." ^ decl_name

let alias decl_name ty : string * Parsetree_rep.decl = decl_name, Mono (Alias ty)

let tuple decl_name fields : string * Parsetree_rep.decl = decl_name, Mono (Tuple fields)

let record decl_name ?(src_id = parsetree decl_name) fields
  : string * Parsetree_rep.decl
  =
  decl_name, Mono (Record { source = src_id; fields })

let variant decl_name ?(src_id = parsetree decl_name) clauses
  : string * Parsetree_rep.decl
  =
  decl_name, Mono (Variant { source = src_id; clauses })

let flag decl_name tags =
  variant decl_name ~src_id:("Asttypes." ^ decl_name)
    (List.map tags ~f:(fun tag -> tag, []))

let inst decl_name ty_name ty : string * Parsetree_rep.decl =
  decl_name, Inst { poly = ty_name; arg = ty }

let poly decl_name rep : string * Parsetree_rep.decl = decl_name, Poly rep
...
let t =
  ...
  ; poly "include_infos"
      (Record
         { source = "Parsetree.include_infos"
         ; fields =
             [ "pincl_mod", A ()
             ; "pincl_loc", Location
