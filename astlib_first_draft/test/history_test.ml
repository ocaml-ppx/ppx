...
let%expect_test "versions" =
  ...
  |> Astlib_first_draft_ast.History.to_versioned_grammars
  |> List.iter ~f:(fun (version, _) ->
    print_endline version);
  [%expect {|
    V4_07 |}]

let rec sexp_of_data : Astlib_first_draft_ast.Grammar.data -> Sexp.t = function
...
let sexp_of_field ({ field_name; data } : Astlib_first_draft_ast.Grammar.field) =
  [%sexp (field_name : string), (data : data)]

let sexp_of_clause ({ clause_name; fields } : Astlib_first_draft_ast.Grammar.clause) =
  [%sexp (clause_name : string), (fields : field list)]

let sexp_of_kind ({ kind_name; clauses } : Astlib_first_draft_ast.Grammar.kind) =
...
type conversion = Astlib_first_draft_ast.History.conversion

let sexp_of_conversion (conversion : conversion) : Sexp.t = Atom conversion.name

type conversion_step = Astlib_first_draft_ast.History.conversion_step =
  { src_version : string
  ; dst_version : string
