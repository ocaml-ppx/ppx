...
let print_parsetree_test_ml () =
  ...
  |> List.filter_map ~f:(fun (name, _) ->
    if List.mem Astlib_first_draft_parsetree_types.entry_points name ~equal:String.equal
    then Some name
    else None)
      ...
      Print.indented (fun () ->
        ...
        Print.indented (fun () ->
          ...
          Print.indented (fun () ->
            ...
            Print.format "(Astlib_first_draft.Conversions.%s_of_ast" name;
            Print.indented (fun () ->
              (Print.format "(Astlib_first_draft.Conversions.%s_to_ast x))));" name))))));
      Print.format "[%%expect {| |}]"))
