  ...
  List.iteri types ~f:(fun index (name, decl) ->
    ...
    Print.format "%s %s%s =%s"
      ...
      (if decl_is_generative decl
       then Printf.sprintf " %sAstlib_first_draft_parsetree.%s =" arg name
       else "");
    Print.indented (fun () ->
...
let print_clause_generator_bindings clauses =
  List.iteri clauses ~f:(fun clause_index (name, tys) ->
    ...
    Print.indented (fun () ->
      ...
      then Print.format "return Astlib_first_draft_parsetree.%s" name
      else (
        let fields = tuple_fields tys in
        let_bind fields;
        Print.format "Astlib_first_draft_parsetree.%s (%s)"
          name
...
let print_parsetree_extended_ml () =
  ...
  print_parsetree_types Astlib_first_draft_parsetree_types.t;
  print_parsetree_generators Astlib_first_draft_parsetree_types.t;
  print_parsetree_modules Astlib_first_draft_parsetree_types.t
