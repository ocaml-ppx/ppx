    ...
    Print.declare_module version (fun () ->
      List.iteri grammar ~f:(fun kind_index (kind : Astlib_first_draft_ast.Grammar.kind) ->
        ...
        Print.declare_module kind.kind_name (fun () ->
          Print.format "val copy : Astlib_first_draft.%s.%s.t -> Astlib_first_draft.%s.%s.t"
            version
            kind.kind_name
            version
            kind.kind_name))))

let rec copy_data (data : Astlib_first_draft_ast.Grammar.data) =
  match data with
        ...
        (List.map grammar ~f:(fun (kind : Astlib_first_draft_ast.Grammar.kind) ->
           ...
           let body () =
             ...
             List.iter kind.clauses ~f:(fun (clause : Astlib_first_draft_ast.Grammar.clause) ->
               ...
               Print.indented (fun () ->
                 List.iter clause.fields ~f:(fun (field : Astlib_first_draft_ast.Grammar.field) ->
                   Print.format "let %s = %s%s in"
                     ...
                     (match copy_data field.data with
                      ...
                      | None -> "")
                     field.field_name);
                 Print.format "Astlib_first_draft.%s.%s.of_concrete (%s%s)"
                   version
                   kind.kind_name
                   clause.clause_name
                   (match clause.fields with
