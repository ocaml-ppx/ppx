(*$ Astlib_first_draft_src_cinaps.print_conversions_ml () *)
open! StdLabels

let rec longident_of_ast x =
  let of_concrete : Stable.V4_07.Longident.Concrete.t -> Astlib_first_draft_parsetree.longident option =
    function
    | Lident { a } ->
      Some (Lident (a) : Astlib_first_draft_parsetree.longident)
    | Ldot { a; b } ->
      Optional.bind (longident_of_ast a) ~f:(fun a ->
        Some (Ldot (a, b) : Astlib_first_draft_parsetree.longident))
    | Lapply { a; b } ->
      Optional.bind (longident_of_ast a) ~f:(fun a ->
        Optional.bind (longident_of_ast b) ~f:(fun b ->
          Some (Lapply (a, b) : Astlib_first_draft_parsetree.longident)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Longident.to_concrete x)

and longident_loc_of_ast x =
  let of_concrete : Stable.V4_07.Longident_loc.Concrete.t -> Astlib_first_draft_parsetree.longident_loc option =
    fun (Longident_loc { txt; loc }) ->
      Optional.bind (longident_of_ast txt) ~f:(fun txt ->
        Some ({ txt; loc } : Astlib_first_draft_parsetree.longident_loc))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Longident_loc.to_concrete x)

and rec_flag_of_ast x =
  let of_concrete : Stable.V4_07.Rec_flag.Concrete.t -> Astlib_first_draft_parsetree.rec_flag option =
    function
    | Nonrecursive ->
      Some (Nonrecursive : Astlib_first_draft_parsetree.rec_flag)
    | Recursive ->
      Some (Recursive : Astlib_first_draft_parsetree.rec_flag)
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Rec_flag.to_concrete x)

and direction_flag_of_ast x =
  let of_concrete : Stable.V4_07.Direction_flag.Concrete.t -> Astlib_first_draft_parsetree.direction_flag option =
    function
    | Upto ->
      Some (Upto : Astlib_first_draft_parsetree.direction_flag)
    | Downto ->
      Some (Downto : Astlib_first_draft_parsetree.direction_flag)
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Direction_flag.to_concrete x)

and private_flag_of_ast x =
  let of_concrete : Stable.V4_07.Private_flag.Concrete.t -> Astlib_first_draft_parsetree.private_flag option =
    function
    | Private ->
      Some (Private : Astlib_first_draft_parsetree.private_flag)
    | Public ->
      Some (Public : Astlib_first_draft_parsetree.private_flag)
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Private_flag.to_concrete x)

and mutable_flag_of_ast x =
  let of_concrete : Stable.V4_07.Mutable_flag.Concrete.t -> Astlib_first_draft_parsetree.mutable_flag option =
    function
    | Immutable ->
      Some (Immutable : Astlib_first_draft_parsetree.mutable_flag)
    | Mutable ->
      Some (Mutable : Astlib_first_draft_parsetree.mutable_flag)
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Mutable_flag.to_concrete x)

and virtual_flag_of_ast x =
  let of_concrete : Stable.V4_07.Virtual_flag.Concrete.t -> Astlib_first_draft_parsetree.virtual_flag option =
    function
    | Virtual ->
      Some (Virtual : Astlib_first_draft_parsetree.virtual_flag)
    | Concrete ->
      Some (Concrete : Astlib_first_draft_parsetree.virtual_flag)
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Virtual_flag.to_concrete x)

and override_flag_of_ast x =
  let of_concrete : Stable.V4_07.Override_flag.Concrete.t -> Astlib_first_draft_parsetree.override_flag option =
    function
    | Override ->
      Some (Override : Astlib_first_draft_parsetree.override_flag)
    | Fresh ->
      Some (Fresh : Astlib_first_draft_parsetree.override_flag)
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Override_flag.to_concrete x)

and closed_flag_of_ast x =
  let of_concrete : Stable.V4_07.Closed_flag.Concrete.t -> Astlib_first_draft_parsetree.closed_flag option =
    function
    | Closed ->
      Some (Closed : Astlib_first_draft_parsetree.closed_flag)
    | Open ->
      Some (Open : Astlib_first_draft_parsetree.closed_flag)
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Closed_flag.to_concrete x)

and label_of_ast x =
  let of_concrete : Stable.V4_07.Label.Concrete.t -> Astlib_first_draft_parsetree.label option =
    fun (Label { a }) ->
      Some a
  in
...
and label_loc_of_ast x =
  let of_concrete : Stable.V4_07.Label_loc.Concrete.t -> Astlib_first_draft_parsetree.label_loc option =
    fun (Label_loc { txt; loc }) ->
      Optional.bind (label_of_ast txt) ~f:(fun txt ->
        Some ({ txt; loc } : Astlib_first_draft_parsetree.label_loc))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Label_loc.to_concrete x)

and string_loc_of_ast x =
  let of_concrete : Stable.V4_07.String_loc.Concrete.t -> Astlib_first_draft_parsetree.string_loc option =
    fun (String_loc { txt; loc }) ->
      Some ({ txt; loc } : Astlib_first_draft_parsetree.string_loc)
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.String_loc.to_concrete x)

and arg_label_of_ast x =
  let of_concrete : Stable.V4_07.Arg_label.Concrete.t -> Astlib_first_draft_parsetree.arg_label option =
    function
    | Nolabel ->
      Some (Nolabel : Astlib_first_draft_parsetree.arg_label)
    | Labelled { a } ->
      Some (Labelled (a) : Astlib_first_draft_parsetree.arg_label)
    | Optional { a } ->
      Some (Optional (a) : Astlib_first_draft_parsetree.arg_label)
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Arg_label.to_concrete x)

and variance_of_ast x =
  let of_concrete : Stable.V4_07.Variance.Concrete.t -> Astlib_first_draft_parsetree.variance option =
    function
    | Covariant ->
      Some (Covariant : Astlib_first_draft_parsetree.variance)
    | Contravariant ->
      Some (Contravariant : Astlib_first_draft_parsetree.variance)
    | Invariant ->
      Some (Invariant : Astlib_first_draft_parsetree.variance)
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Variance.to_concrete x)

and constant_of_ast x =
  let of_concrete : Stable.V4_07.Constant.Concrete.t -> Astlib_first_draft_parsetree.constant option =
    function
    | Pconst_integer { a; b } ->
      Some (Pconst_integer (a, b) : Astlib_first_draft_parsetree.constant)
    | Pconst_char { a } ->
      Some (Pconst_char (a) : Astlib_first_draft_parsetree.constant)
    | Pconst_string { a; b } ->
      Some (Pconst_string (a, b) : Astlib_first_draft_parsetree.constant)
    | Pconst_float { a; b } ->
      Some (Pconst_float (a, b) : Astlib_first_draft_parsetree.constant)
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Constant.to_concrete x)

and attribute_of_ast x =
  let of_concrete : Stable.V4_07.Attribute.Concrete.t -> Astlib_first_draft_parsetree.attribute option =
    fun (Attribute { a; b }) ->
...
and extension_of_ast x =
  let of_concrete : Stable.V4_07.Extension.Concrete.t -> Astlib_first_draft_parsetree.extension option =
    fun (Extension { a; b }) ->
      Optional.bind (string_loc_of_ast a) ~f:(fun a ->
        Optional.bind (payload_of_ast b) ~f:(fun b ->
          Some (a, b)))
...
and attributes_of_ast x =
  let of_concrete : Stable.V4_07.Attributes.Concrete.t -> Astlib_first_draft_parsetree.attributes option =
    fun (Attributes { a }) ->
...
and payload_of_ast x =
  let of_concrete : Stable.V4_07.Payload.Concrete.t -> Astlib_first_draft_parsetree.payload option =
    function
    | PStr { a } ->
      Optional.bind (structure_of_ast a) ~f:(fun a ->
        Some (PStr (a) : Astlib_first_draft_parsetree.payload))
    | PSig { a } ->
      Optional.bind (signature_of_ast a) ~f:(fun a ->
        Some (PSig (a) : Astlib_first_draft_parsetree.payload))
    | PTyp { a } ->
      Optional.bind (core_type_of_ast a) ~f:(fun a ->
        Some (PTyp (a) : Astlib_first_draft_parsetree.payload))
    | PPat { a; b } ->
      Optional.bind (pattern_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.Option.map ~f:(expression_of_ast) b) ~f:(fun b ->
          Some (PPat (a, b) : Astlib_first_draft_parsetree.payload)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Payload.to_concrete x)

and core_type_of_ast x =
  let of_concrete : Stable.V4_07.Core_type.Concrete.t -> Astlib_first_draft_parsetree.core_type option =
    fun (Core_type { ptyp_desc; ptyp_loc; ptyp_attributes }) ->
      Optional.bind (core_type_desc_of_ast ptyp_desc) ~f:(fun ptyp_desc ->
        Optional.bind (attributes_of_ast ptyp_attributes) ~f:(fun ptyp_attributes ->
          Some ({ ptyp_desc; ptyp_loc; ptyp_attributes } : Astlib_first_draft_parsetree.core_type)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Core_type.to_concrete x)

and core_type_desc_of_ast x =
  let of_concrete : Stable.V4_07.Core_type_desc.Concrete.t -> Astlib_first_draft_parsetree.core_type_desc option =
    function
    | Ptyp_any ->
      Some (Ptyp_any : Astlib_first_draft_parsetree.core_type_desc)
    | Ptyp_var { a } ->
      Some (Ptyp_var (a) : Astlib_first_draft_parsetree.core_type_desc)
    | Ptyp_arrow { a; b; c } ->
      Optional.bind (arg_label_of_ast a) ~f:(fun a ->
        Optional.bind (core_type_of_ast b) ~f:(fun b ->
          Optional.bind (core_type_of_ast c) ~f:(fun c ->
            Some (Ptyp_arrow (a, b, c) : Astlib_first_draft_parsetree.core_type_desc))))
    | Ptyp_tuple { a } ->
      Optional.bind (Optional.List.map ~f:(core_type_of_ast) a) ~f:(fun a ->
        Some (Ptyp_tuple (a) : Astlib_first_draft_parsetree.core_type_desc))
    | Ptyp_constr { a; b } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.List.map ~f:(core_type_of_ast) b) ~f:(fun b ->
          Some (Ptyp_constr (a, b) : Astlib_first_draft_parsetree.core_type_desc)))
    | Ptyp_object { a; b } ->
      Optional.bind (Optional.List.map ~f:(object_field_of_ast) a) ~f:(fun a ->
        Optional.bind (closed_flag_of_ast b) ~f:(fun b ->
          Some (Ptyp_object (a, b) : Astlib_first_draft_parsetree.core_type_desc)))
    | Ptyp_class { a; b } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.List.map ~f:(core_type_of_ast) b) ~f:(fun b ->
          Some (Ptyp_class (a, b) : Astlib_first_draft_parsetree.core_type_desc)))
    | Ptyp_alias { a; b } ->
      Optional.bind (core_type_of_ast a) ~f:(fun a ->
        Some (Ptyp_alias (a, b) : Astlib_first_draft_parsetree.core_type_desc))
    | Ptyp_variant { a; b; c } ->
      Optional.bind (Optional.List.map ~f:(row_field_of_ast) a) ~f:(fun a ->
        Optional.bind (closed_flag_of_ast b) ~f:(fun b ->
          Optional.bind (Optional.Option.map ~f:(Optional.List.map ~f:(label_of_ast)) c) ~f:(fun c ->
            Some (Ptyp_variant (a, b, c) : Astlib_first_draft_parsetree.core_type_desc))))
    | Ptyp_poly { a; b } ->
      Optional.bind (Optional.List.map ~f:(string_loc_of_ast) a) ~f:(fun a ->
        Optional.bind (core_type_of_ast b) ~f:(fun b ->
          Some (Ptyp_poly (a, b) : Astlib_first_draft_parsetree.core_type_desc)))
    | Ptyp_package { a } ->
      Optional.bind (package_type_of_ast a) ~f:(fun a ->
        Some (Ptyp_package (a) : Astlib_first_draft_parsetree.core_type_desc))
    | Ptyp_extension { a } ->
      Optional.bind (extension_of_ast a) ~f:(fun a ->
        Some (Ptyp_extension (a) : Astlib_first_draft_parsetree.core_type_desc))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Core_type_desc.to_concrete x)

and package_type_of_ast x =
  let of_concrete : Stable.V4_07.Package_type.Concrete.t -> Astlib_first_draft_parsetree.package_type option =
...
and package_type_constraint_of_ast x =
  let of_concrete : Stable.V4_07.Package_type_constraint.Concrete.t -> Astlib_first_draft_parsetree.package_type_constraint option =
...
and row_field_of_ast x =
  let of_concrete : Stable.V4_07.Row_field.Concrete.t -> Astlib_first_draft_parsetree.row_field option =
    function
    | Rtag { a; b; c; d } ->
      Optional.bind (label_loc_of_ast a) ~f:(fun a ->
        Optional.bind (attributes_of_ast b) ~f:(fun b ->
          Optional.bind (Optional.List.map ~f:(core_type_of_ast) d) ~f:(fun d ->
            Some (Rtag (a, b, c, d) : Astlib_first_draft_parsetree.row_field))))
    | Rinherit { a } ->
      Optional.bind (core_type_of_ast a) ~f:(fun a ->
        Some (Rinherit (a) : Astlib_first_draft_parsetree.row_field))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Row_field.to_concrete x)

and object_field_of_ast x =
  let of_concrete : Stable.V4_07.Object_field.Concrete.t -> Astlib_first_draft_parsetree.object_field option =
    function
    | Otag { a; b; c } ->
      Optional.bind (label_loc_of_ast a) ~f:(fun a ->
        Optional.bind (attributes_of_ast b) ~f:(fun b ->
          Optional.bind (core_type_of_ast c) ~f:(fun c ->
            Some (Otag (a, b, c) : Astlib_first_draft_parsetree.object_field))))
    | Oinherit { a } ->
      Optional.bind (core_type_of_ast a) ~f:(fun a ->
        Some (Oinherit (a) : Astlib_first_draft_parsetree.object_field))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Object_field.to_concrete x)

and pattern_of_ast x =
  let of_concrete : Stable.V4_07.Pattern.Concrete.t -> Astlib_first_draft_parsetree.pattern option =
    fun (Pattern { ppat_desc; ppat_loc; ppat_attributes }) ->
      Optional.bind (pattern_desc_of_ast ppat_desc) ~f:(fun ppat_desc ->
        Optional.bind (attributes_of_ast ppat_attributes) ~f:(fun ppat_attributes ->
          Some ({ ppat_desc; ppat_loc; ppat_attributes } : Astlib_first_draft_parsetree.pattern)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Pattern.to_concrete x)

and pattern_desc_of_ast x =
  let of_concrete : Stable.V4_07.Pattern_desc.Concrete.t -> Astlib_first_draft_parsetree.pattern_desc option =
    function
    | Ppat_any ->
      Some (Ppat_any : Astlib_first_draft_parsetree.pattern_desc)
    | Ppat_var { a } ->
      Optional.bind (string_loc_of_ast a) ~f:(fun a ->
        Some (Ppat_var (a) : Astlib_first_draft_parsetree.pattern_desc))
    | Ppat_alias { a; b } ->
      Optional.bind (pattern_of_ast a) ~f:(fun a ->
        Optional.bind (string_loc_of_ast b) ~f:(fun b ->
          Some (Ppat_alias (a, b) : Astlib_first_draft_parsetree.pattern_desc)))
    | Ppat_constant { a } ->
      Optional.bind (constant_of_ast a) ~f:(fun a ->
        Some (Ppat_constant (a) : Astlib_first_draft_parsetree.pattern_desc))
    | Ppat_interval { a; b } ->
      Optional.bind (constant_of_ast a) ~f:(fun a ->
        Optional.bind (constant_of_ast b) ~f:(fun b ->
          Some (Ppat_interval (a, b) : Astlib_first_draft_parsetree.pattern_desc)))
    | Ppat_tuple { a } ->
      Optional.bind (Optional.List.map ~f:(pattern_of_ast) a) ~f:(fun a ->
        Some (Ppat_tuple (a) : Astlib_first_draft_parsetree.pattern_desc))
    | Ppat_construct { a; b } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.Option.map ~f:(pattern_of_ast) b) ~f:(fun b ->
          Some (Ppat_construct (a, b) : Astlib_first_draft_parsetree.pattern_desc)))
    | Ppat_variant { a; b } ->
      Optional.bind (label_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.Option.map ~f:(pattern_of_ast) b) ~f:(fun b ->
          Some (Ppat_variant (a, b) : Astlib_first_draft_parsetree.pattern_desc)))
    | Ppat_record { a; b } ->
      Optional.bind (Optional.List.map ~f:(record_field_pattern_of_ast) a) ~f:(fun a ->
        Optional.bind (closed_flag_of_ast b) ~f:(fun b ->
          Some (Ppat_record (a, b) : Astlib_first_draft_parsetree.pattern_desc)))
    | Ppat_array { a } ->
      Optional.bind (Optional.List.map ~f:(pattern_of_ast) a) ~f:(fun a ->
        Some (Ppat_array (a) : Astlib_first_draft_parsetree.pattern_desc))
    | Ppat_or { a; b } ->
      Optional.bind (pattern_of_ast a) ~f:(fun a ->
        Optional.bind (pattern_of_ast b) ~f:(fun b ->
          Some (Ppat_or (a, b) : Astlib_first_draft_parsetree.pattern_desc)))
    | Ppat_constraint { a; b } ->
      Optional.bind (pattern_of_ast a) ~f:(fun a ->
        Optional.bind (core_type_of_ast b) ~f:(fun b ->
          Some (Ppat_constraint (a, b) : Astlib_first_draft_parsetree.pattern_desc)))
    | Ppat_type { a } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Some (Ppat_type (a) : Astlib_first_draft_parsetree.pattern_desc))
    | Ppat_lazy { a } ->
      Optional.bind (pattern_of_ast a) ~f:(fun a ->
        Some (Ppat_lazy (a) : Astlib_first_draft_parsetree.pattern_desc))
    | Ppat_unpack { a } ->
      Optional.bind (string_loc_of_ast a) ~f:(fun a ->
        Some (Ppat_unpack (a) : Astlib_first_draft_parsetree.pattern_desc))
    | Ppat_exception { a } ->
      Optional.bind (pattern_of_ast a) ~f:(fun a ->
        Some (Ppat_exception (a) : Astlib_first_draft_parsetree.pattern_desc))
    | Ppat_extension { a } ->
      Optional.bind (extension_of_ast a) ~f:(fun a ->
        Some (Ppat_extension (a) : Astlib_first_draft_parsetree.pattern_desc))
    | Ppat_open { a; b } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Optional.bind (pattern_of_ast b) ~f:(fun b ->
          Some (Ppat_open (a, b) : Astlib_first_draft_parsetree.pattern_desc)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Pattern_desc.to_concrete x)

and record_field_pattern_of_ast x =
  let of_concrete : Stable.V4_07.Record_field_pattern.Concrete.t -> Astlib_first_draft_parsetree.record_field_pattern option =
...
and expression_of_ast x =
  let of_concrete : Stable.V4_07.Expression.Concrete.t -> Astlib_first_draft_parsetree.expression option =
    fun (Expression { pexp_desc; pexp_loc; pexp_attributes }) ->
      Optional.bind (expression_desc_of_ast pexp_desc) ~f:(fun pexp_desc ->
        Optional.bind (attributes_of_ast pexp_attributes) ~f:(fun pexp_attributes ->
          Some ({ pexp_desc; pexp_loc; pexp_attributes } : Astlib_first_draft_parsetree.expression)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Expression.to_concrete x)

and expression_desc_of_ast x =
  let of_concrete : Stable.V4_07.Expression_desc.Concrete.t -> Astlib_first_draft_parsetree.expression_desc option =
    function
    | Pexp_ident { a } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Some (Pexp_ident (a) : Astlib_first_draft_parsetree.expression_desc))
    | Pexp_constant { a } ->
      Optional.bind (constant_of_ast a) ~f:(fun a ->
        Some (Pexp_constant (a) : Astlib_first_draft_parsetree.expression_desc))
    | Pexp_let { a; b; c } ->
      Optional.bind (rec_flag_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.List.map ~f:(value_binding_of_ast) b) ~f:(fun b ->
          Optional.bind (expression_of_ast c) ~f:(fun c ->
            Some (Pexp_let (a, b, c) : Astlib_first_draft_parsetree.expression_desc))))
    | Pexp_function { a } ->
      Optional.bind (Optional.List.map ~f:(case_of_ast) a) ~f:(fun a ->
        Some (Pexp_function (a) : Astlib_first_draft_parsetree.expression_desc))
    | Pexp_fun { a; b; c; d } ->
      Optional.bind (arg_label_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.Option.map ~f:(expression_of_ast) b) ~f:(fun b ->
          Optional.bind (pattern_of_ast c) ~f:(fun c ->
            Optional.bind (expression_of_ast d) ~f:(fun d ->
              Some (Pexp_fun (a, b, c, d) : Astlib_first_draft_parsetree.expression_desc)))))
    | Pexp_apply { a; b } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.List.map ~f:(apply_arg_of_ast) b) ~f:(fun b ->
          Some (Pexp_apply (a, b) : Astlib_first_draft_parsetree.expression_desc)))
    | Pexp_match { a; b } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.List.map ~f:(case_of_ast) b) ~f:(fun b ->
          Some (Pexp_match (a, b) : Astlib_first_draft_parsetree.expression_desc)))
    | Pexp_try { a; b } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.List.map ~f:(case_of_ast) b) ~f:(fun b ->
          Some (Pexp_try (a, b) : Astlib_first_draft_parsetree.expression_desc)))
    | Pexp_tuple { a } ->
      Optional.bind (Optional.List.map ~f:(expression_of_ast) a) ~f:(fun a ->
        Some (Pexp_tuple (a) : Astlib_first_draft_parsetree.expression_desc))
    | Pexp_construct { a; b } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.Option.map ~f:(expression_of_ast) b) ~f:(fun b ->
          Some (Pexp_construct (a, b) : Astlib_first_draft_parsetree.expression_desc)))
    | Pexp_variant { a; b } ->
      Optional.bind (label_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.Option.map ~f:(expression_of_ast) b) ~f:(fun b ->
          Some (Pexp_variant (a, b) : Astlib_first_draft_parsetree.expression_desc)))
    | Pexp_record { a; b } ->
      Optional.bind (Optional.List.map ~f:(record_field_expression_of_ast) a) ~f:(fun a ->
        Optional.bind (Optional.Option.map ~f:(expression_of_ast) b) ~f:(fun b ->
          Some (Pexp_record (a, b) : Astlib_first_draft_parsetree.expression_desc)))
    | Pexp_field { a; b } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (longident_loc_of_ast b) ~f:(fun b ->
          Some (Pexp_field (a, b) : Astlib_first_draft_parsetree.expression_desc)))
    | Pexp_setfield { a; b; c } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (longident_loc_of_ast b) ~f:(fun b ->
          Optional.bind (expression_of_ast c) ~f:(fun c ->
            Some (Pexp_setfield (a, b, c) : Astlib_first_draft_parsetree.expression_desc))))
    | Pexp_array { a } ->
      Optional.bind (Optional.List.map ~f:(expression_of_ast) a) ~f:(fun a ->
        Some (Pexp_array (a) : Astlib_first_draft_parsetree.expression_desc))
      ...
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (expression_of_ast b) ~f:(fun b ->
          Optional.bind (Optional.Option.map ~f:(expression_of_ast) c) ~f:(fun c ->
            Some (Pexp_ifthenelse (a, b, c) : Astlib_first_draft_parsetree.expression_desc))))
    | Pexp_sequence { a; b } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (expression_of_ast b) ~f:(fun b ->
          Some (Pexp_sequence (a, b) : Astlib_first_draft_parsetree.expression_desc)))
    | Pexp_while { a; b } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (expression_of_ast b) ~f:(fun b ->
          Some (Pexp_while (a, b) : Astlib_first_draft_parsetree.expression_desc)))
    | Pexp_for { a; b; c; d; e } ->
      Optional.bind (pattern_of_ast a) ~f:(fun a ->
        Optional.bind (expression_of_ast b) ~f:(fun b ->
          Optional.bind (expression_of_ast c) ~f:(fun c ->
            Optional.bind (direction_flag_of_ast d) ~f:(fun d ->
              Optional.bind (expression_of_ast e) ~f:(fun e ->
                Some (Pexp_for (a, b, c, d, e) : Astlib_first_draft_parsetree.expression_desc))))))
    | Pexp_constraint { a; b } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (core_type_of_ast b) ~f:(fun b ->
          Some (Pexp_constraint (a, b) : Astlib_first_draft_parsetree.expression_desc)))
    | Pexp_coerce { a; b; c } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.Option.map ~f:(core_type_of_ast) b) ~f:(fun b ->
          Optional.bind (core_type_of_ast c) ~f:(fun c ->
            Some (Pexp_coerce (a, b, c) : Astlib_first_draft_parsetree.expression_desc))))
    | Pexp_send { a; b } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (label_loc_of_ast b) ~f:(fun b ->
          Some (Pexp_send (a, b) : Astlib_first_draft_parsetree.expression_desc)))
    | Pexp_new { a } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Some (Pexp_new (a) : Astlib_first_draft_parsetree.expression_desc))
    | Pexp_setinstvar { a; b } ->
      Optional.bind (label_loc_of_ast a) ~f:(fun a ->
        Optional.bind (expression_of_ast b) ~f:(fun b ->
          Some (Pexp_setinstvar (a, b) : Astlib_first_draft_parsetree.expression_desc)))
    | Pexp_override { a } ->
      Optional.bind (Optional.List.map ~f:(override_expression_of_ast) a) ~f:(fun a ->
        Some (Pexp_override (a) : Astlib_first_draft_parsetree.expression_desc))
    | Pexp_letmodule { a; b; c } ->
      Optional.bind (string_loc_of_ast a) ~f:(fun a ->
        Optional.bind (module_expr_of_ast b) ~f:(fun b ->
          Optional.bind (expression_of_ast c) ~f:(fun c ->
            Some (Pexp_letmodule (a, b, c) : Astlib_first_draft_parsetree.expression_desc))))
    | Pexp_letexception { a; b } ->
      Optional.bind (extension_constructor_of_ast a) ~f:(fun a ->
        Optional.bind (expression_of_ast b) ~f:(fun b ->
          Some (Pexp_letexception (a, b) : Astlib_first_draft_parsetree.expression_desc)))
    | Pexp_assert { a } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Some (Pexp_assert (a) : Astlib_first_draft_parsetree.expression_desc))
    | Pexp_lazy { a } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Some (Pexp_lazy (a) : Astlib_first_draft_parsetree.expression_desc))
    | Pexp_poly { a; b } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.Option.map ~f:(core_type_of_ast) b) ~f:(fun b ->
          Some (Pexp_poly (a, b) : Astlib_first_draft_parsetree.expression_desc)))
    | Pexp_object { a } ->
      Optional.bind (class_structure_of_ast a) ~f:(fun a ->
        Some (Pexp_object (a) : Astlib_first_draft_parsetree.expression_desc))
    | Pexp_newtype { a; b } ->
      Optional.bind (string_loc_of_ast a) ~f:(fun a ->
        Optional.bind (expression_of_ast b) ~f:(fun b ->
          Some (Pexp_newtype (a, b) : Astlib_first_draft_parsetree.expression_desc)))
    | Pexp_pack { a } ->
      Optional.bind (module_expr_of_ast a) ~f:(fun a ->
        Some (Pexp_pack (a) : Astlib_first_draft_parsetree.expression_desc))
    | Pexp_open { a; b; c } ->
      Optional.bind (override_flag_of_ast a) ~f:(fun a ->
        Optional.bind (longident_loc_of_ast b) ~f:(fun b ->
          Optional.bind (expression_of_ast c) ~f:(fun c ->
            Some (Pexp_open (a, b, c) : Astlib_first_draft_parsetree.expression_desc))))
    | Pexp_extension { a } ->
      Optional.bind (extension_of_ast a) ~f:(fun a ->
        Some (Pexp_extension (a) : Astlib_first_draft_parsetree.expression_desc))
    | Pexp_unreachable ->
      Some (Pexp_unreachable : Astlib_first_draft_parsetree.expression_desc)
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Expression_desc.to_concrete x)

and override_expression_of_ast x =
  let of_concrete : Stable.V4_07.Override_expression.Concrete.t -> Astlib_first_draft_parsetree.override_expression option =
...
and record_field_expression_of_ast x =
  let of_concrete : Stable.V4_07.Record_field_expression.Concrete.t -> Astlib_first_draft_parsetree.record_field_expression option =
...
and apply_arg_of_ast x =
  let of_concrete : Stable.V4_07.Apply_arg.Concrete.t -> Astlib_first_draft_parsetree.apply_arg option =
    fun (Apply_arg { a; b }) ->
      Optional.bind (arg_label_of_ast a) ~f:(fun a ->
        Optional.bind (expression_of_ast b) ~f:(fun b ->
          Some (a, b)))
...
and case_of_ast x =
  let of_concrete : Stable.V4_07.Case.Concrete.t -> Astlib_first_draft_parsetree.case option =
      ...
      Optional.bind (pattern_of_ast pc_lhs) ~f:(fun pc_lhs ->
        Optional.bind (Optional.Option.map ~f:(expression_of_ast) pc_guard) ~f:(fun pc_guard ->
          Optional.bind (expression_of_ast pc_rhs) ~f:(fun pc_rhs ->
            Some ({ pc_lhs; pc_guard; pc_rhs } : Astlib_first_draft_parsetree.case))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Case.to_concrete x)

and value_description_of_ast x =
  let of_concrete : Stable.V4_07.Value_description.Concrete.t -> Astlib_first_draft_parsetree.value_description option =
      ...
      Optional.bind (string_loc_of_ast pval_name) ~f:(fun pval_name ->
        Optional.bind (core_type_of_ast pval_type) ~f:(fun pval_type ->
          Optional.bind (attributes_of_ast pval_attributes) ~f:(fun pval_attributes ->
            Some ({ pval_name; pval_type; pval_prim; pval_attributes; pval_loc } : Astlib_first_draft_parsetree.value_description))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Value_description.to_concrete x)

and type_declaration_of_ast x =
  let of_concrete : Stable.V4_07.Type_declaration.Concrete.t -> Astlib_first_draft_parsetree.type_declaration option =
              ...
              Optional.bind (private_flag_of_ast ptype_private) ~f:(fun ptype_private ->
                Optional.bind (Optional.Option.map ~f:(core_type_of_ast) ptype_manifest) ~f:(fun ptype_manifest ->
                  Optional.bind (attributes_of_ast ptype_attributes) ~f:(fun ptype_attributes ->
                    Some ({ ptype_name; ptype_params; ptype_cstrs; ptype_kind; ptype_private; ptype_manifest; ptype_attributes; ptype_loc } : Astlib_first_draft_parsetree.type_declaration))))))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Type_declaration.to_concrete x)

and type_param_of_ast x =
  let of_concrete : Stable.V4_07.Type_param.Concrete.t -> Astlib_first_draft_parsetree.type_param option =
    fun (Type_param { a; b }) ->
      Optional.bind (core_type_of_ast a) ~f:(fun a ->
...
and type_constraint_of_ast x =
  let of_concrete : Stable.V4_07.Type_constraint.Concrete.t -> Astlib_first_draft_parsetree.type_constraint option =
...
and type_kind_of_ast x =
  let of_concrete : Stable.V4_07.Type_kind.Concrete.t -> Astlib_first_draft_parsetree.type_kind option =
    function
    | Ptype_abstract ->
      Some (Ptype_abstract : Astlib_first_draft_parsetree.type_kind)
    | Ptype_variant { a } ->
      Optional.bind (Optional.List.map ~f:(constructor_declaration_of_ast) a) ~f:(fun a ->
        Some (Ptype_variant (a) : Astlib_first_draft_parsetree.type_kind))
    | Ptype_record { a } ->
      Optional.bind (Optional.List.map ~f:(label_declaration_of_ast) a) ~f:(fun a ->
        Some (Ptype_record (a) : Astlib_first_draft_parsetree.type_kind))
    | Ptype_open ->
      Some (Ptype_open : Astlib_first_draft_parsetree.type_kind)
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Type_kind.to_concrete x)

and label_declaration_of_ast x =
  let of_concrete : Stable.V4_07.Label_declaration.Concrete.t -> Astlib_first_draft_parsetree.label_declaration option =
        ...
        Optional.bind (mutable_flag_of_ast pld_mutable) ~f:(fun pld_mutable ->
          Optional.bind (core_type_of_ast pld_type) ~f:(fun pld_type ->
            Optional.bind (attributes_of_ast pld_attributes) ~f:(fun pld_attributes ->
              Some ({ pld_name; pld_mutable; pld_type; pld_loc; pld_attributes } : Astlib_first_draft_parsetree.label_declaration)))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Label_declaration.to_concrete x)

and constructor_declaration_of_ast x =
  let of_concrete : Stable.V4_07.Constructor_declaration.Concrete.t -> Astlib_first_draft_parsetree.constructor_declaration option =
        ...
        Optional.bind (constructor_arguments_of_ast pcd_args) ~f:(fun pcd_args ->
          Optional.bind (Optional.Option.map ~f:(core_type_of_ast) pcd_res) ~f:(fun pcd_res ->
            Optional.bind (attributes_of_ast pcd_attributes) ~f:(fun pcd_attributes ->
              Some ({ pcd_name; pcd_args; pcd_res; pcd_loc; pcd_attributes } : Astlib_first_draft_parsetree.constructor_declaration)))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Constructor_declaration.to_concrete x)

and constructor_arguments_of_ast x =
  let of_concrete : Stable.V4_07.Constructor_arguments.Concrete.t -> Astlib_first_draft_parsetree.constructor_arguments option =
    function
    | Pcstr_tuple { a } ->
      Optional.bind (Optional.List.map ~f:(core_type_of_ast) a) ~f:(fun a ->
        Some (Pcstr_tuple (a) : Astlib_first_draft_parsetree.constructor_arguments))
    | Pcstr_record { a } ->
      Optional.bind (Optional.List.map ~f:(label_declaration_of_ast) a) ~f:(fun a ->
        Some (Pcstr_record (a) : Astlib_first_draft_parsetree.constructor_arguments))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Constructor_arguments.to_concrete x)

and type_extension_of_ast x =
  let of_concrete : Stable.V4_07.Type_extension.Concrete.t -> Astlib_first_draft_parsetree.type_extension option =
          ...
          Optional.bind (Optional.List.map ~f:(extension_constructor_of_ast) ptyext_constructors) ~f:(fun ptyext_constructors ->
            Optional.bind (private_flag_of_ast ptyext_private) ~f:(fun ptyext_private ->
              Optional.bind (attributes_of_ast ptyext_attributes) ~f:(fun ptyext_attributes ->
                Some ({ ptyext_path; ptyext_params; ptyext_constructors; ptyext_private; ptyext_attributes } : Astlib_first_draft_parsetree.type_extension))))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Type_extension.to_concrete x)

and extension_constructor_of_ast x =
  let of_concrete : Stable.V4_07.Extension_constructor.Concrete.t -> Astlib_first_draft_parsetree.extension_constructor option =
      ...
      Optional.bind (string_loc_of_ast pext_name) ~f:(fun pext_name ->
        Optional.bind (extension_constructor_kind_of_ast pext_kind) ~f:(fun pext_kind ->
          Optional.bind (attributes_of_ast pext_attributes) ~f:(fun pext_attributes ->
            Some ({ pext_name; pext_kind; pext_loc; pext_attributes } : Astlib_first_draft_parsetree.extension_constructor))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Extension_constructor.to_concrete x)

and extension_constructor_kind_of_ast x =
  let of_concrete : Stable.V4_07.Extension_constructor_kind.Concrete.t -> Astlib_first_draft_parsetree.extension_constructor_kind option =
    function
    | Pext_decl { a; b } ->
      Optional.bind (constructor_arguments_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.Option.map ~f:(core_type_of_ast) b) ~f:(fun b ->
          Some (Pext_decl (a, b) : Astlib_first_draft_parsetree.extension_constructor_kind)))
    | Pext_rebind { a } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Some (Pext_rebind (a) : Astlib_first_draft_parsetree.extension_constructor_kind))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Extension_constructor_kind.to_concrete x)

and class_type_of_ast x =
  let of_concrete : Stable.V4_07.Class_type.Concrete.t -> Astlib_first_draft_parsetree.class_type option =
    fun (Class_type { pcty_desc; pcty_loc; pcty_attributes }) ->
      Optional.bind (class_type_desc_of_ast pcty_desc) ~f:(fun pcty_desc ->
        Optional.bind (attributes_of_ast pcty_attributes) ~f:(fun pcty_attributes ->
          Some ({ pcty_desc; pcty_loc; pcty_attributes } : Astlib_first_draft_parsetree.class_type)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Class_type.to_concrete x)

and class_type_desc_of_ast x =
  let of_concrete : Stable.V4_07.Class_type_desc.Concrete.t -> Astlib_first_draft_parsetree.class_type_desc option =
    function
    | Pcty_constr { a; b } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.List.map ~f:(core_type_of_ast) b) ~f:(fun b ->
          Some (Pcty_constr (a, b) : Astlib_first_draft_parsetree.class_type_desc)))
    | Pcty_signature { a } ->
      Optional.bind (class_signature_of_ast a) ~f:(fun a ->
        Some (Pcty_signature (a) : Astlib_first_draft_parsetree.class_type_desc))
    | Pcty_arrow { a; b; c } ->
      Optional.bind (arg_label_of_ast a) ~f:(fun a ->
        Optional.bind (core_type_of_ast b) ~f:(fun b ->
          Optional.bind (class_type_of_ast c) ~f:(fun c ->
            Some (Pcty_arrow (a, b, c) : Astlib_first_draft_parsetree.class_type_desc))))
    | Pcty_extension { a } ->
      Optional.bind (extension_of_ast a) ~f:(fun a ->
        Some (Pcty_extension (a) : Astlib_first_draft_parsetree.class_type_desc))
    | Pcty_open { a; b; c } ->
      Optional.bind (override_flag_of_ast a) ~f:(fun a ->
        Optional.bind (longident_loc_of_ast b) ~f:(fun b ->
          Optional.bind (class_type_of_ast c) ~f:(fun c ->
            Some (Pcty_open (a, b, c) : Astlib_first_draft_parsetree.class_type_desc))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Class_type_desc.to_concrete x)

and class_signature_of_ast x =
  let of_concrete : Stable.V4_07.Class_signature.Concrete.t -> Astlib_first_draft_parsetree.class_signature option =
    fun (Class_signature { pcsig_self; pcsig_fields }) ->
      Optional.bind (core_type_of_ast pcsig_self) ~f:(fun pcsig_self ->
        Optional.bind (Optional.List.map ~f:(class_type_field_of_ast) pcsig_fields) ~f:(fun pcsig_fields ->
          Some ({ pcsig_self; pcsig_fields } : Astlib_first_draft_parsetree.class_signature)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Class_signature.to_concrete x)

and class_type_field_of_ast x =
  let of_concrete : Stable.V4_07.Class_type_field.Concrete.t -> Astlib_first_draft_parsetree.class_type_field option =
    fun (Class_type_field { pctf_desc; pctf_loc; pctf_attributes }) ->
      Optional.bind (class_type_field_desc_of_ast pctf_desc) ~f:(fun pctf_desc ->
        Optional.bind (attributes_of_ast pctf_attributes) ~f:(fun pctf_attributes ->
          Some ({ pctf_desc; pctf_loc; pctf_attributes } : Astlib_first_draft_parsetree.class_type_field)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Class_type_field.to_concrete x)

and class_type_field_desc_of_ast x =
  let of_concrete : Stable.V4_07.Class_type_field_desc.Concrete.t -> Astlib_first_draft_parsetree.class_type_field_desc option =
    function
    | Pctf_inherit { a } ->
      Optional.bind (class_type_of_ast a) ~f:(fun a ->
        Some (Pctf_inherit (a) : Astlib_first_draft_parsetree.class_type_field_desc))
    | Pctf_val { a } ->
      Optional.bind (class_type_value_desc_of_ast a) ~f:(fun a ->
        Some (Pctf_val (a) : Astlib_first_draft_parsetree.class_type_field_desc))
    | Pctf_method { a } ->
      Optional.bind (class_type_method_desc_of_ast a) ~f:(fun a ->
        Some (Pctf_method (a) : Astlib_first_draft_parsetree.class_type_field_desc))
    | Pctf_constraint { a } ->
      Optional.bind (class_type_constraint_of_ast a) ~f:(fun a ->
        Some (Pctf_constraint (a) : Astlib_first_draft_parsetree.class_type_field_desc))
    | Pctf_attribute { a } ->
      Optional.bind (attribute_of_ast a) ~f:(fun a ->
        Some (Pctf_attribute (a) : Astlib_first_draft_parsetree.class_type_field_desc))
    | Pctf_extension { a } ->
      Optional.bind (extension_of_ast a) ~f:(fun a ->
        Some (Pctf_extension (a) : Astlib_first_draft_parsetree.class_type_field_desc))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Class_type_field_desc.to_concrete x)

and class_type_value_desc_of_ast x =
  let of_concrete : Stable.V4_07.Class_type_value_desc.Concrete.t -> Astlib_first_draft_parsetree.class_type_value_desc option =
...
and class_type_method_desc_of_ast x =
  let of_concrete : Stable.V4_07.Class_type_method_desc.Concrete.t -> Astlib_first_draft_parsetree.class_type_method_desc option =
...
and class_type_constraint_of_ast x =
  let of_concrete : Stable.V4_07.Class_type_constraint.Concrete.t -> Astlib_first_draft_parsetree.class_type_constraint option =
...
and class_description_of_ast x =
  let of_concrete : Stable.V4_07.Class_description.Concrete.t -> Astlib_first_draft_parsetree.class_description option =
          ...
          Optional.bind (string_loc_of_ast pci_name) ~f:(fun pci_name ->
            Optional.bind (class_type_of_ast pci_expr) ~f:(fun pci_expr ->
              Optional.bind (attributes_of_ast pci_attributes) ~f:(fun pci_attributes ->
                Some ({ pci_virt; pci_params; pci_name; pci_expr; pci_loc; pci_attributes } : Astlib_first_draft_parsetree.class_description))))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Class_description.to_concrete x)

and class_type_declaration_of_ast x =
  let of_concrete : Stable.V4_07.Class_type_declaration.Concrete.t -> Astlib_first_draft_parsetree.class_type_declaration option =
      ...
      Optional.bind (virtual_flag_of_ast pci_virt) ~f:(fun pci_virt ->
        Optional.bind (Optional.List.map ~f:(type_param_of_ast) pci_params) ~f:(fun pci_params ->
          Optional.bind (string_loc_of_ast pci_name) ~f:(fun pci_name ->
            Optional.bind (class_type_of_ast pci_expr) ~f:(fun pci_expr ->
              Optional.bind (attributes_of_ast pci_attributes) ~f:(fun pci_attributes ->
                Some ({ pci_virt; pci_params; pci_name; pci_expr; pci_loc; pci_attributes } : Astlib_first_draft_parsetree.class_type_declaration))))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Class_type_declaration.to_concrete x)

and class_expr_of_ast x =
  let of_concrete : Stable.V4_07.Class_expr.Concrete.t -> Astlib_first_draft_parsetree.class_expr option =
    fun (Class_expr { pcl_desc; pcl_loc; pcl_attributes }) ->
      Optional.bind (class_expr_desc_of_ast pcl_desc) ~f:(fun pcl_desc ->
        Optional.bind (attributes_of_ast pcl_attributes) ~f:(fun pcl_attributes ->
          Some ({ pcl_desc; pcl_loc; pcl_attributes } : Astlib_first_draft_parsetree.class_expr)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Class_expr.to_concrete x)

and class_expr_desc_of_ast x =
  let of_concrete : Stable.V4_07.Class_expr_desc.Concrete.t -> Astlib_first_draft_parsetree.class_expr_desc option =
    function
    | Pcl_constr { a; b } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.List.map ~f:(core_type_of_ast) b) ~f:(fun b ->
          Some (Pcl_constr (a, b) : Astlib_first_draft_parsetree.class_expr_desc)))
    | Pcl_structure { a } ->
      Optional.bind (class_structure_of_ast a) ~f:(fun a ->
        Some (Pcl_structure (a) : Astlib_first_draft_parsetree.class_expr_desc))
    | Pcl_fun { a; b; c; d } ->
      Optional.bind (arg_label_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.Option.map ~f:(expression_of_ast) b) ~f:(fun b ->
          Optional.bind (pattern_of_ast c) ~f:(fun c ->
            Optional.bind (class_expr_of_ast d) ~f:(fun d ->
              Some (Pcl_fun (a, b, c, d) : Astlib_first_draft_parsetree.class_expr_desc)))))
    | Pcl_apply { a; b } ->
      Optional.bind (class_expr_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.List.map ~f:(apply_arg_of_ast) b) ~f:(fun b ->
          Some (Pcl_apply (a, b) : Astlib_first_draft_parsetree.class_expr_desc)))
    | Pcl_let { a; b; c } ->
      Optional.bind (rec_flag_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.List.map ~f:(value_binding_of_ast) b) ~f:(fun b ->
          Optional.bind (class_expr_of_ast c) ~f:(fun c ->
            Some (Pcl_let (a, b, c) : Astlib_first_draft_parsetree.class_expr_desc))))
    | Pcl_constraint { a; b } ->
      Optional.bind (class_expr_of_ast a) ~f:(fun a ->
        Optional.bind (class_type_of_ast b) ~f:(fun b ->
          Some (Pcl_constraint (a, b) : Astlib_first_draft_parsetree.class_expr_desc)))
    | Pcl_extension { a } ->
      Optional.bind (extension_of_ast a) ~f:(fun a ->
        Some (Pcl_extension (a) : Astlib_first_draft_parsetree.class_expr_desc))
    | Pcl_open { a; b; c } ->
      Optional.bind (override_flag_of_ast a) ~f:(fun a ->
        Optional.bind (longident_loc_of_ast b) ~f:(fun b ->
          Optional.bind (class_expr_of_ast c) ~f:(fun c ->
            Some (Pcl_open (a, b, c) : Astlib_first_draft_parsetree.class_expr_desc))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Class_expr_desc.to_concrete x)

and class_structure_of_ast x =
  let of_concrete : Stable.V4_07.Class_structure.Concrete.t -> Astlib_first_draft_parsetree.class_structure option =
    fun (Class_structure { pcstr_self; pcstr_fields }) ->
      Optional.bind (pattern_of_ast pcstr_self) ~f:(fun pcstr_self ->
        Optional.bind (Optional.List.map ~f:(class_field_of_ast) pcstr_fields) ~f:(fun pcstr_fields ->
          Some ({ pcstr_self; pcstr_fields } : Astlib_first_draft_parsetree.class_structure)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Class_structure.to_concrete x)

and class_field_of_ast x =
  let of_concrete : Stable.V4_07.Class_field.Concrete.t -> Astlib_first_draft_parsetree.class_field option =
    fun (Class_field { pcf_desc; pcf_loc; pcf_attributes }) ->
      Optional.bind (class_field_desc_of_ast pcf_desc) ~f:(fun pcf_desc ->
        Optional.bind (attributes_of_ast pcf_attributes) ~f:(fun pcf_attributes ->
          Some ({ pcf_desc; pcf_loc; pcf_attributes } : Astlib_first_draft_parsetree.class_field)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Class_field.to_concrete x)

and class_field_desc_of_ast x =
  let of_concrete : Stable.V4_07.Class_field_desc.Concrete.t -> Astlib_first_draft_parsetree.class_field_desc option =
    function
    | Pcf_inherit { a; b; c } ->
      Optional.bind (override_flag_of_ast a) ~f:(fun a ->
        Optional.bind (class_expr_of_ast b) ~f:(fun b ->
          Optional.bind (Optional.Option.map ~f:(string_loc_of_ast) c) ~f:(fun c ->
            Some (Pcf_inherit (a, b, c) : Astlib_first_draft_parsetree.class_field_desc))))
    | Pcf_val { a } ->
      Optional.bind (class_value_desc_of_ast a) ~f:(fun a ->
        Some (Pcf_val (a) : Astlib_first_draft_parsetree.class_field_desc))
    | Pcf_method { a } ->
      Optional.bind (class_method_desc_of_ast a) ~f:(fun a ->
        Some (Pcf_method (a) : Astlib_first_draft_parsetree.class_field_desc))
    | Pcf_constraint { a } ->
      Optional.bind (class_type_constraint_of_ast a) ~f:(fun a ->
        Some (Pcf_constraint (a) : Astlib_first_draft_parsetree.class_field_desc))
    | Pcf_initializer { a } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Some (Pcf_initializer (a) : Astlib_first_draft_parsetree.class_field_desc))
    | Pcf_attribute { a } ->
      Optional.bind (attribute_of_ast a) ~f:(fun a ->
        Some (Pcf_attribute (a) : Astlib_first_draft_parsetree.class_field_desc))
    | Pcf_extension { a } ->
      Optional.bind (extension_of_ast a) ~f:(fun a ->
        Some (Pcf_extension (a) : Astlib_first_draft_parsetree.class_field_desc))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Class_field_desc.to_concrete x)

and class_value_desc_of_ast x =
  let of_concrete : Stable.V4_07.Class_value_desc.Concrete.t -> Astlib_first_draft_parsetree.class_value_desc option =
...
and class_method_desc_of_ast x =
  let of_concrete : Stable.V4_07.Class_method_desc.Concrete.t -> Astlib_first_draft_parsetree.class_method_desc option =
...
and class_field_kind_of_ast x =
  let of_concrete : Stable.V4_07.Class_field_kind.Concrete.t -> Astlib_first_draft_parsetree.class_field_kind option =
    function
    | Cfk_virtual { a } ->
      Optional.bind (core_type_of_ast a) ~f:(fun a ->
        Some (Cfk_virtual (a) : Astlib_first_draft_parsetree.class_field_kind))
    | Cfk_concrete { a; b } ->
      Optional.bind (override_flag_of_ast a) ~f:(fun a ->
        Optional.bind (expression_of_ast b) ~f:(fun b ->
          Some (Cfk_concrete (a, b) : Astlib_first_draft_parsetree.class_field_kind)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Class_field_kind.to_concrete x)

and class_declaration_of_ast x =
  let of_concrete : Stable.V4_07.Class_declaration.Concrete.t -> Astlib_first_draft_parsetree.class_declaration option =
        ...
        Optional.bind (Optional.List.map ~f:(type_param_of_ast) pci_params) ~f:(fun pci_params ->
          Optional.bind (string_loc_of_ast pci_name) ~f:(fun pci_name ->
            Optional.bind (class_expr_of_ast pci_expr) ~f:(fun pci_expr ->
              Optional.bind (attributes_of_ast pci_attributes) ~f:(fun pci_attributes ->
                Some ({ pci_virt; pci_params; pci_name; pci_expr; pci_loc; pci_attributes } : Astlib_first_draft_parsetree.class_declaration))))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Class_declaration.to_concrete x)

and module_type_of_ast x =
  let of_concrete : Stable.V4_07.Module_type.Concrete.t -> Astlib_first_draft_parsetree.module_type option =
    fun (Module_type { pmty_desc; pmty_loc; pmty_attributes }) ->
      Optional.bind (module_type_desc_of_ast pmty_desc) ~f:(fun pmty_desc ->
        Optional.bind (attributes_of_ast pmty_attributes) ~f:(fun pmty_attributes ->
          Some ({ pmty_desc; pmty_loc; pmty_attributes } : Astlib_first_draft_parsetree.module_type)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Module_type.to_concrete x)

and module_type_desc_of_ast x =
  let of_concrete : Stable.V4_07.Module_type_desc.Concrete.t -> Astlib_first_draft_parsetree.module_type_desc option =
    function
    | Pmty_ident { a } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Some (Pmty_ident (a) : Astlib_first_draft_parsetree.module_type_desc))
    | Pmty_signature { a } ->
      Optional.bind (signature_of_ast a) ~f:(fun a ->
        Some (Pmty_signature (a) : Astlib_first_draft_parsetree.module_type_desc))
    | Pmty_functor { a; b; c } ->
      Optional.bind (string_loc_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.Option.map ~f:(module_type_of_ast) b) ~f:(fun b ->
          Optional.bind (module_type_of_ast c) ~f:(fun c ->
            Some (Pmty_functor (a, b, c) : Astlib_first_draft_parsetree.module_type_desc))))
    | Pmty_with { a; b } ->
      Optional.bind (module_type_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.List.map ~f:(with_constraint_of_ast) b) ~f:(fun b ->
          Some (Pmty_with (a, b) : Astlib_first_draft_parsetree.module_type_desc)))
    | Pmty_typeof { a } ->
      Optional.bind (module_expr_of_ast a) ~f:(fun a ->
        Some (Pmty_typeof (a) : Astlib_first_draft_parsetree.module_type_desc))
    | Pmty_extension { a } ->
      Optional.bind (extension_of_ast a) ~f:(fun a ->
        Some (Pmty_extension (a) : Astlib_first_draft_parsetree.module_type_desc))
    | Pmty_alias { a } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Some (Pmty_alias (a) : Astlib_first_draft_parsetree.module_type_desc))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Module_type_desc.to_concrete x)

and signature_of_ast x =
  let of_concrete : Stable.V4_07.Signature.Concrete.t -> Astlib_first_draft_parsetree.signature option =
    fun (Signature { a }) ->
...
and signature_item_of_ast x =
  let of_concrete : Stable.V4_07.Signature_item.Concrete.t -> Astlib_first_draft_parsetree.signature_item option =
    fun (Signature_item { psig_desc; psig_loc }) ->
      Optional.bind (signature_item_desc_of_ast psig_desc) ~f:(fun psig_desc ->
        Some ({ psig_desc; psig_loc } : Astlib_first_draft_parsetree.signature_item))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Signature_item.to_concrete x)

and signature_item_desc_of_ast x =
  let of_concrete : Stable.V4_07.Signature_item_desc.Concrete.t -> Astlib_first_draft_parsetree.signature_item_desc option =
    function
    | Psig_value { a } ->
      Optional.bind (value_description_of_ast a) ~f:(fun a ->
        Some (Psig_value (a) : Astlib_first_draft_parsetree.signature_item_desc))
    | Psig_type { a; b } ->
      Optional.bind (rec_flag_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.List.map ~f:(type_declaration_of_ast) b) ~f:(fun b ->
          Some (Psig_type (a, b) : Astlib_first_draft_parsetree.signature_item_desc)))
    | Psig_typext { a } ->
      Optional.bind (type_extension_of_ast a) ~f:(fun a ->
        Some (Psig_typext (a) : Astlib_first_draft_parsetree.signature_item_desc))
    | Psig_exception { a } ->
      Optional.bind (extension_constructor_of_ast a) ~f:(fun a ->
        Some (Psig_exception (a) : Astlib_first_draft_parsetree.signature_item_desc))
    | Psig_module { a } ->
      Optional.bind (module_declaration_of_ast a) ~f:(fun a ->
        Some (Psig_module (a) : Astlib_first_draft_parsetree.signature_item_desc))
    | Psig_recmodule { a } ->
      Optional.bind (Optional.List.map ~f:(module_declaration_of_ast) a) ~f:(fun a ->
        Some (Psig_recmodule (a) : Astlib_first_draft_parsetree.signature_item_desc))
    | Psig_modtype { a } ->
      Optional.bind (module_type_declaration_of_ast a) ~f:(fun a ->
        Some (Psig_modtype (a) : Astlib_first_draft_parsetree.signature_item_desc))
    | Psig_open { a } ->
      Optional.bind (open_description_of_ast a) ~f:(fun a ->
        Some (Psig_open (a) : Astlib_first_draft_parsetree.signature_item_desc))
    | Psig_include { a } ->
      Optional.bind (include_description_of_ast a) ~f:(fun a ->
        Some (Psig_include (a) : Astlib_first_draft_parsetree.signature_item_desc))
    | Psig_class { a } ->
      Optional.bind (Optional.List.map ~f:(class_description_of_ast) a) ~f:(fun a ->
        Some (Psig_class (a) : Astlib_first_draft_parsetree.signature_item_desc))
    | Psig_class_type { a } ->
      Optional.bind (Optional.List.map ~f:(class_type_declaration_of_ast) a) ~f:(fun a ->
        Some (Psig_class_type (a) : Astlib_first_draft_parsetree.signature_item_desc))
    | Psig_attribute { a } ->
      Optional.bind (attribute_of_ast a) ~f:(fun a ->
        Some (Psig_attribute (a) : Astlib_first_draft_parsetree.signature_item_desc))
    | Psig_extension { a; b } ->
      Optional.bind (extension_of_ast a) ~f:(fun a ->
        Optional.bind (attributes_of_ast b) ~f:(fun b ->
          Some (Psig_extension (a, b) : Astlib_first_draft_parsetree.signature_item_desc)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Signature_item_desc.to_concrete x)

and module_declaration_of_ast x =
  let of_concrete : Stable.V4_07.Module_declaration.Concrete.t -> Astlib_first_draft_parsetree.module_declaration option =
      ...
      Optional.bind (string_loc_of_ast pmd_name) ~f:(fun pmd_name ->
        Optional.bind (module_type_of_ast pmd_type) ~f:(fun pmd_type ->
          Optional.bind (attributes_of_ast pmd_attributes) ~f:(fun pmd_attributes ->
            Some ({ pmd_name; pmd_type; pmd_attributes; pmd_loc } : Astlib_first_draft_parsetree.module_declaration))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Module_declaration.to_concrete x)

and module_type_declaration_of_ast x =
  let of_concrete : Stable.V4_07.Module_type_declaration.Concrete.t -> Astlib_first_draft_parsetree.module_type_declaration option =
      ...
      Optional.bind (string_loc_of_ast pmtd_name) ~f:(fun pmtd_name ->
        Optional.bind (Optional.Option.map ~f:(module_type_of_ast) pmtd_type) ~f:(fun pmtd_type ->
          Optional.bind (attributes_of_ast pmtd_attributes) ~f:(fun pmtd_attributes ->
            Some ({ pmtd_name; pmtd_type; pmtd_attributes; pmtd_loc } : Astlib_first_draft_parsetree.module_type_declaration))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Module_type_declaration.to_concrete x)

and open_description_of_ast x =
  let of_concrete : Stable.V4_07.Open_description.Concrete.t -> Astlib_first_draft_parsetree.open_description option =
      ...
      Optional.bind (longident_loc_of_ast popen_lid) ~f:(fun popen_lid ->
        Optional.bind (override_flag_of_ast popen_override) ~f:(fun popen_override ->
          Optional.bind (attributes_of_ast popen_attributes) ~f:(fun popen_attributes ->
            Some ({ popen_lid; popen_override; popen_loc; popen_attributes } : Astlib_first_draft_parsetree.open_description))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Open_description.to_concrete x)

and include_description_of_ast x =
  let of_concrete : Stable.V4_07.Include_description.Concrete.t -> Astlib_first_draft_parsetree.include_description option =
    fun (Include_description { pincl_mod; pincl_loc; pincl_attributes }) ->
      Optional.bind (module_type_of_ast pincl_mod) ~f:(fun pincl_mod ->
        Optional.bind (attributes_of_ast pincl_attributes) ~f:(fun pincl_attributes ->
          Some ({ pincl_mod; pincl_loc; pincl_attributes } : Astlib_first_draft_parsetree.include_description)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Include_description.to_concrete x)

and include_declaration_of_ast x =
  let of_concrete : Stable.V4_07.Include_declaration.Concrete.t -> Astlib_first_draft_parsetree.include_declaration option =
    fun (Include_declaration { pincl_mod; pincl_loc; pincl_attributes }) ->
      Optional.bind (module_expr_of_ast pincl_mod) ~f:(fun pincl_mod ->
        Optional.bind (attributes_of_ast pincl_attributes) ~f:(fun pincl_attributes ->
          Some ({ pincl_mod; pincl_loc; pincl_attributes } : Astlib_first_draft_parsetree.include_declaration)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Include_declaration.to_concrete x)

and with_constraint_of_ast x =
  let of_concrete : Stable.V4_07.With_constraint.Concrete.t -> Astlib_first_draft_parsetree.with_constraint option =
    function
    | Pwith_type { a; b } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Optional.bind (type_declaration_of_ast b) ~f:(fun b ->
          Some (Pwith_type (a, b) : Astlib_first_draft_parsetree.with_constraint)))
    | Pwith_module { a; b } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Optional.bind (longident_loc_of_ast b) ~f:(fun b ->
          Some (Pwith_module (a, b) : Astlib_first_draft_parsetree.with_constraint)))
    | Pwith_typesubst { a; b } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Optional.bind (type_declaration_of_ast b) ~f:(fun b ->
          Some (Pwith_typesubst (a, b) : Astlib_first_draft_parsetree.with_constraint)))
    | Pwith_modsubst { a; b } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Optional.bind (longident_loc_of_ast b) ~f:(fun b ->
          Some (Pwith_modsubst (a, b) : Astlib_first_draft_parsetree.with_constraint)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.With_constraint.to_concrete x)

and module_expr_of_ast x =
  let of_concrete : Stable.V4_07.Module_expr.Concrete.t -> Astlib_first_draft_parsetree.module_expr option =
    fun (Module_expr { pmod_desc; pmod_loc; pmod_attributes }) ->
      Optional.bind (module_expr_desc_of_ast pmod_desc) ~f:(fun pmod_desc ->
        Optional.bind (attributes_of_ast pmod_attributes) ~f:(fun pmod_attributes ->
          Some ({ pmod_desc; pmod_loc; pmod_attributes } : Astlib_first_draft_parsetree.module_expr)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Module_expr.to_concrete x)

and module_expr_desc_of_ast x =
  let of_concrete : Stable.V4_07.Module_expr_desc.Concrete.t -> Astlib_first_draft_parsetree.module_expr_desc option =
    function
    | Pmod_ident { a } ->
      Optional.bind (longident_loc_of_ast a) ~f:(fun a ->
        Some (Pmod_ident (a) : Astlib_first_draft_parsetree.module_expr_desc))
    | Pmod_structure { a } ->
      Optional.bind (structure_of_ast a) ~f:(fun a ->
        Some (Pmod_structure (a) : Astlib_first_draft_parsetree.module_expr_desc))
    | Pmod_functor { a; b; c } ->
      Optional.bind (string_loc_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.Option.map ~f:(module_type_of_ast) b) ~f:(fun b ->
          Optional.bind (module_expr_of_ast c) ~f:(fun c ->
            Some (Pmod_functor (a, b, c) : Astlib_first_draft_parsetree.module_expr_desc))))
    | Pmod_apply { a; b } ->
      Optional.bind (module_expr_of_ast a) ~f:(fun a ->
        Optional.bind (module_expr_of_ast b) ~f:(fun b ->
          Some (Pmod_apply (a, b) : Astlib_first_draft_parsetree.module_expr_desc)))
    | Pmod_constraint { a; b } ->
      Optional.bind (module_expr_of_ast a) ~f:(fun a ->
        Optional.bind (module_type_of_ast b) ~f:(fun b ->
          Some (Pmod_constraint (a, b) : Astlib_first_draft_parsetree.module_expr_desc)))
    | Pmod_unpack { a } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Some (Pmod_unpack (a) : Astlib_first_draft_parsetree.module_expr_desc))
    | Pmod_extension { a } ->
      Optional.bind (extension_of_ast a) ~f:(fun a ->
        Some (Pmod_extension (a) : Astlib_first_draft_parsetree.module_expr_desc))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Module_expr_desc.to_concrete x)

and structure_of_ast x =
  let of_concrete : Stable.V4_07.Structure.Concrete.t -> Astlib_first_draft_parsetree.structure option =
    fun (Structure { a }) ->
...
and structure_item_of_ast x =
  let of_concrete : Stable.V4_07.Structure_item.Concrete.t -> Astlib_first_draft_parsetree.structure_item option =
    fun (Structure_item { pstr_desc; pstr_loc }) ->
      Optional.bind (structure_item_desc_of_ast pstr_desc) ~f:(fun pstr_desc ->
        Some ({ pstr_desc; pstr_loc } : Astlib_first_draft_parsetree.structure_item))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Structure_item.to_concrete x)

and structure_item_desc_of_ast x =
  let of_concrete : Stable.V4_07.Structure_item_desc.Concrete.t -> Astlib_first_draft_parsetree.structure_item_desc option =
    function
    | Pstr_eval { a; b } ->
      Optional.bind (expression_of_ast a) ~f:(fun a ->
        Optional.bind (attributes_of_ast b) ~f:(fun b ->
          Some (Pstr_eval (a, b) : Astlib_first_draft_parsetree.structure_item_desc)))
    | Pstr_value { a; b } ->
      Optional.bind (rec_flag_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.List.map ~f:(value_binding_of_ast) b) ~f:(fun b ->
          Some (Pstr_value (a, b) : Astlib_first_draft_parsetree.structure_item_desc)))
    | Pstr_primitive { a } ->
      Optional.bind (value_description_of_ast a) ~f:(fun a ->
        Some (Pstr_primitive (a) : Astlib_first_draft_parsetree.structure_item_desc))
    | Pstr_type { a; b } ->
      Optional.bind (rec_flag_of_ast a) ~f:(fun a ->
        Optional.bind (Optional.List.map ~f:(type_declaration_of_ast) b) ~f:(fun b ->
          Some (Pstr_type (a, b) : Astlib_first_draft_parsetree.structure_item_desc)))
    | Pstr_typext { a } ->
      Optional.bind (type_extension_of_ast a) ~f:(fun a ->
        Some (Pstr_typext (a) : Astlib_first_draft_parsetree.structure_item_desc))
    | Pstr_exception { a } ->
      Optional.bind (extension_constructor_of_ast a) ~f:(fun a ->
        Some (Pstr_exception (a) : Astlib_first_draft_parsetree.structure_item_desc))
    | Pstr_module { a } ->
      Optional.bind (module_binding_of_ast a) ~f:(fun a ->
        Some (Pstr_module (a) : Astlib_first_draft_parsetree.structure_item_desc))
    | Pstr_recmodule { a } ->
      Optional.bind (Optional.List.map ~f:(module_binding_of_ast) a) ~f:(fun a ->
        Some (Pstr_recmodule (a) : Astlib_first_draft_parsetree.structure_item_desc))
    | Pstr_modtype { a } ->
      Optional.bind (module_type_declaration_of_ast a) ~f:(fun a ->
        Some (Pstr_modtype (a) : Astlib_first_draft_parsetree.structure_item_desc))
    | Pstr_open { a } ->
      Optional.bind (open_description_of_ast a) ~f:(fun a ->
        Some (Pstr_open (a) : Astlib_first_draft_parsetree.structure_item_desc))
    | Pstr_class { a } ->
      Optional.bind (Optional.List.map ~f:(class_declaration_of_ast) a) ~f:(fun a ->
        Some (Pstr_class (a) : Astlib_first_draft_parsetree.structure_item_desc))
    | Pstr_class_type { a } ->
      Optional.bind (Optional.List.map ~f:(class_type_declaration_of_ast) a) ~f:(fun a ->
        Some (Pstr_class_type (a) : Astlib_first_draft_parsetree.structure_item_desc))
    | Pstr_include { a } ->
      Optional.bind (include_declaration_of_ast a) ~f:(fun a ->
        Some (Pstr_include (a) : Astlib_first_draft_parsetree.structure_item_desc))
    | Pstr_attribute { a } ->
      Optional.bind (attribute_of_ast a) ~f:(fun a ->
        Some (Pstr_attribute (a) : Astlib_first_draft_parsetree.structure_item_desc))
    | Pstr_extension { a; b } ->
      Optional.bind (extension_of_ast a) ~f:(fun a ->
        Optional.bind (attributes_of_ast b) ~f:(fun b ->
          Some (Pstr_extension (a, b) : Astlib_first_draft_parsetree.structure_item_desc)))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Structure_item_desc.to_concrete x)

and value_binding_of_ast x =
  let of_concrete : Stable.V4_07.Value_binding.Concrete.t -> Astlib_first_draft_parsetree.value_binding option =
      ...
      Optional.bind (pattern_of_ast pvb_pat) ~f:(fun pvb_pat ->
        Optional.bind (expression_of_ast pvb_expr) ~f:(fun pvb_expr ->
          Optional.bind (attributes_of_ast pvb_attributes) ~f:(fun pvb_attributes ->
            Some ({ pvb_pat; pvb_expr; pvb_attributes; pvb_loc } : Astlib_first_draft_parsetree.value_binding))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Value_binding.to_concrete x)

and module_binding_of_ast x =
  let of_concrete : Stable.V4_07.Module_binding.Concrete.t -> Astlib_first_draft_parsetree.module_binding option =
      ...
      Optional.bind (string_loc_of_ast pmb_name) ~f:(fun pmb_name ->
        Optional.bind (module_expr_of_ast pmb_expr) ~f:(fun pmb_expr ->
          Optional.bind (attributes_of_ast pmb_attributes) ~f:(fun pmb_attributes ->
            Some ({ pmb_name; pmb_expr; pmb_attributes; pmb_loc } : Astlib_first_draft_parsetree.module_binding))))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Module_binding.to_concrete x)

and toplevel_phrase_of_ast x =
  let of_concrete : Stable.V4_07.Toplevel_phrase.Concrete.t -> Astlib_first_draft_parsetree.toplevel_phrase option =
    function
    | Ptop_def { a } ->
      Optional.bind (structure_of_ast a) ~f:(fun a ->
        Some (Ptop_def (a) : Astlib_first_draft_parsetree.toplevel_phrase))
    | Ptop_dir { a; b } ->
      Optional.bind (directive_argument_of_ast b) ~f:(fun b ->
        Some (Ptop_dir (a, b) : Astlib_first_draft_parsetree.toplevel_phrase))
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Toplevel_phrase.to_concrete x)

and directive_argument_of_ast x =
  let of_concrete : Stable.V4_07.Directive_argument.Concrete.t -> Astlib_first_draft_parsetree.directive_argument option =
    function
    | Pdir_none ->
      Some (Pdir_none : Astlib_first_draft_parsetree.directive_argument)
    | Pdir_string { a } ->
      Some (Pdir_string (a) : Astlib_first_draft_parsetree.directive_argument)
    | Pdir_int { a; b } ->
      Some (Pdir_int (a, b) : Astlib_first_draft_parsetree.directive_argument)
    | Pdir_ident { a } ->
      Optional.bind (longident_of_ast a) ~f:(fun a ->
        Some (Pdir_ident (a) : Astlib_first_draft_parsetree.directive_argument))
    | Pdir_bool { a } ->
      Some (Pdir_bool (a) : Astlib_first_draft_parsetree.directive_argument)
  in
  Optional.bind ~f:of_concrete (Stable.V4_07.Directive_argument.to_concrete x)

let rec longident_to_ast : Astlib_first_draft_parsetree.longident -> Stable.V4_07.Longident.t =
  function
  | Lident (a) ->
...
and longident_loc_to_ast : Astlib_first_draft_parsetree.longident_loc -> Stable.V4_07.Longident_loc.t =
  fun { txt; loc } ->
    let txt = longident_to_ast txt in
    Stable.V4_07.Longident_loc.of_concrete (Longident_loc { txt; loc })

and rec_flag_to_ast : Astlib_first_draft_parsetree.rec_flag -> Stable.V4_07.Rec_flag.t =
  function
  | Nonrecursive ->
...
and direction_flag_to_ast : Astlib_first_draft_parsetree.direction_flag -> Stable.V4_07.Direction_flag.t =
  function
  | Upto ->
...
and private_flag_to_ast : Astlib_first_draft_parsetree.private_flag -> Stable.V4_07.Private_flag.t =
  function
  | Private ->
...
and mutable_flag_to_ast : Astlib_first_draft_parsetree.mutable_flag -> Stable.V4_07.Mutable_flag.t =
  function
  | Immutable ->
...
and virtual_flag_to_ast : Astlib_first_draft_parsetree.virtual_flag -> Stable.V4_07.Virtual_flag.t =
  function
  | Virtual ->
...
and override_flag_to_ast : Astlib_first_draft_parsetree.override_flag -> Stable.V4_07.Override_flag.t =
  function
  | Override ->
...
and closed_flag_to_ast : Astlib_first_draft_parsetree.closed_flag -> Stable.V4_07.Closed_flag.t =
  function
  | Closed ->
...
and label_to_ast : Astlib_first_draft_parsetree.label -> Stable.V4_07.Label.t =
  fun a ->
    Stable.V4_07.Label.of_concrete (Label { a })

and label_loc_to_ast : Astlib_first_draft_parsetree.label_loc -> Stable.V4_07.Label_loc.t =
  fun { txt; loc } ->
    let txt = label_to_ast txt in
    Stable.V4_07.Label_loc.of_concrete (Label_loc { txt; loc })

and string_loc_to_ast : Astlib_first_draft_parsetree.string_loc -> Stable.V4_07.String_loc.t =
  fun { txt; loc } ->
    Stable.V4_07.String_loc.of_concrete (String_loc { txt; loc })

and arg_label_to_ast : Astlib_first_draft_parsetree.arg_label -> Stable.V4_07.Arg_label.t =
  function
  | Nolabel ->
...
and variance_to_ast : Astlib_first_draft_parsetree.variance -> Stable.V4_07.Variance.t =
  function
  | Covariant ->
...
and constant_to_ast : Astlib_first_draft_parsetree.constant -> Stable.V4_07.Constant.t =
  function
  | Pconst_integer (a, b) ->
...
and attribute_to_ast : Astlib_first_draft_parsetree.attribute -> Stable.V4_07.Attribute.t =
  fun (a, b) ->
    let a = string_loc_to_ast a in
    let b = payload_to_ast b in
    Stable.V4_07.Attribute.of_concrete (Attribute { a; b })

and extension_to_ast : Astlib_first_draft_parsetree.extension -> Stable.V4_07.Extension.t =
  fun (a, b) ->
    let a = string_loc_to_ast a in
    let b = payload_to_ast b in
    Stable.V4_07.Extension.of_concrete (Extension { a; b })

and attributes_to_ast : Astlib_first_draft_parsetree.attributes -> Stable.V4_07.Attributes.t =
  fun a ->
    let a = List.map ~f:(attribute_to_ast) a in
    Stable.V4_07.Attributes.of_concrete (Attributes { a })

and payload_to_ast : Astlib_first_draft_parsetree.payload -> Stable.V4_07.Payload.t =
  function
  | PStr (a) ->
...
and core_type_to_ast : Astlib_first_draft_parsetree.core_type -> Stable.V4_07.Core_type.t =
...
and core_type_desc_to_ast : Astlib_first_draft_parsetree.core_type_desc -> Stable.V4_07.Core_type_desc.t =
  function
  | Ptyp_any ->
...
and package_type_to_ast : Astlib_first_draft_parsetree.package_type -> Stable.V4_07.Package_type.t =
  fun (a, b) ->
    let a = longident_loc_to_ast a in
    let b = List.map ~f:(package_type_constraint_to_ast) b in
    Stable.V4_07.Package_type.of_concrete (Package_type { a; b })

and package_type_constraint_to_ast : Astlib_first_draft_parsetree.package_type_constraint -> Stable.V4_07.Package_type_constraint.t =
  fun (a, b) ->
    let a = longident_loc_to_ast a in
    let b = core_type_to_ast b in
    Stable.V4_07.Package_type_constraint.of_concrete (Package_type_constraint { a; b })

and row_field_to_ast : Astlib_first_draft_parsetree.row_field -> Stable.V4_07.Row_field.t =
  function
  | Rtag (a, b, c, d) ->
...
and object_field_to_ast : Astlib_first_draft_parsetree.object_field -> Stable.V4_07.Object_field.t =
  function
  | Otag (a, b, c) ->
    let a = label_loc_to_ast a in
    let b = attributes_to_ast b in
...
and pattern_to_ast : Astlib_first_draft_parsetree.pattern -> Stable.V4_07.Pattern.t =
...
and pattern_desc_to_ast : Astlib_first_draft_parsetree.pattern_desc -> Stable.V4_07.Pattern_desc.t =
  function
  | Ppat_any ->
...
and record_field_pattern_to_ast : Astlib_first_draft_parsetree.record_field_pattern -> Stable.V4_07.Record_field_pattern.t =
  fun (a, b) ->
    let a = longident_loc_to_ast a in
    let b = pattern_to_ast b in
    Stable.V4_07.Record_field_pattern.of_concrete (Record_field_pattern { a; b })

and expression_to_ast : Astlib_first_draft_parsetree.expression -> Stable.V4_07.Expression.t =
...
and expression_desc_to_ast : Astlib_first_draft_parsetree.expression_desc -> Stable.V4_07.Expression_desc.t =
  function
  | Pexp_ident (a) ->
    let a = longident_loc_to_ast a in
...
and override_expression_to_ast : Astlib_first_draft_parsetree.override_expression -> Stable.V4_07.Override_expression.t =
  fun (a, b) ->
    let a = label_loc_to_ast a in
    let b = expression_to_ast b in
    Stable.V4_07.Override_expression.of_concrete (Override_expression { a; b })

and record_field_expression_to_ast : Astlib_first_draft_parsetree.record_field_expression -> Stable.V4_07.Record_field_expression.t =
  fun (a, b) ->
    let a = longident_loc_to_ast a in
    let b = expression_to_ast b in
    Stable.V4_07.Record_field_expression.of_concrete (Record_field_expression { a; b })

and apply_arg_to_ast : Astlib_first_draft_parsetree.apply_arg -> Stable.V4_07.Apply_arg.t =
  fun (a, b) ->
    let a = arg_label_to_ast a in
    let b = expression_to_ast b in
    Stable.V4_07.Apply_arg.of_concrete (Apply_arg { a; b })

and case_to_ast : Astlib_first_draft_parsetree.case -> Stable.V4_07.Case.t =
...
and value_description_to_ast : Astlib_first_draft_parsetree.value_description -> Stable.V4_07.Value_description.t =
...
and type_declaration_to_ast : Astlib_first_draft_parsetree.type_declaration -> Stable.V4_07.Type_declaration.t =
...
and type_param_to_ast : Astlib_first_draft_parsetree.type_param -> Stable.V4_07.Type_param.t =
  fun (a, b) ->
    let a = core_type_to_ast a in
    let b = variance_to_ast b in
    Stable.V4_07.Type_param.of_concrete (Type_param { a; b })

and type_constraint_to_ast : Astlib_first_draft_parsetree.type_constraint -> Stable.V4_07.Type_constraint.t =
  fun (a, b, c) ->
    let a = core_type_to_ast a in
    let b = core_type_to_ast b in
    Stable.V4_07.Type_constraint.of_concrete (Type_constraint { a; b; c })

and type_kind_to_ast : Astlib_first_draft_parsetree.type_kind -> Stable.V4_07.Type_kind.t =
  function
  | Ptype_abstract ->
...
and label_declaration_to_ast : Astlib_first_draft_parsetree.label_declaration -> Stable.V4_07.Label_declaration.t =
...
and constructor_declaration_to_ast : Astlib_first_draft_parsetree.constructor_declaration -> Stable.V4_07.Constructor_declaration.t =
...
and constructor_arguments_to_ast : Astlib_first_draft_parsetree.constructor_arguments -> Stable.V4_07.Constructor_arguments.t =
  function
  | Pcstr_tuple (a) ->
    let a = List.map ~f:(core_type_to_ast) a in
...
and type_extension_to_ast : Astlib_first_draft_parsetree.type_extension -> Stable.V4_07.Type_extension.t =
...
and extension_constructor_to_ast : Astlib_first_draft_parsetree.extension_constructor -> Stable.V4_07.Extension_constructor.t =
...
and extension_constructor_kind_to_ast : Astlib_first_draft_parsetree.extension_constructor_kind -> Stable.V4_07.Extension_constructor_kind.t =
  function
  | Pext_decl (a, b) ->
...
and class_type_to_ast : Astlib_first_draft_parsetree.class_type -> Stable.V4_07.Class_type.t =
...
and class_type_desc_to_ast : Astlib_first_draft_parsetree.class_type_desc -> Stable.V4_07.Class_type_desc.t =
  function
  | Pcty_constr (a, b) ->
    let a = longident_loc_to_ast a in
    let b = List.map ~f:(core_type_to_ast) b in
...
and class_signature_to_ast : Astlib_first_draft_parsetree.class_signature -> Stable.V4_07.Class_signature.t =
...
and class_type_field_to_ast : Astlib_first_draft_parsetree.class_type_field -> Stable.V4_07.Class_type_field.t =
...
and class_type_field_desc_to_ast : Astlib_first_draft_parsetree.class_type_field_desc -> Stable.V4_07.Class_type_field_desc.t =
  function
  | Pctf_inherit (a) ->
...
and class_type_value_desc_to_ast : Astlib_first_draft_parsetree.class_type_value_desc -> Stable.V4_07.Class_type_value_desc.t =
  fun (a, b, c, d) ->
    let a = label_loc_to_ast a in
...
and class_type_method_desc_to_ast : Astlib_first_draft_parsetree.class_type_method_desc -> Stable.V4_07.Class_type_method_desc.t =
  fun (a, b, c, d) ->
    let a = label_loc_to_ast a in
...
and class_type_constraint_to_ast : Astlib_first_draft_parsetree.class_type_constraint -> Stable.V4_07.Class_type_constraint.t =
  fun (a, b) ->
    let a = core_type_to_ast a in
    let b = core_type_to_ast b in
    Stable.V4_07.Class_type_constraint.of_concrete (Class_type_constraint { a; b })

and class_description_to_ast : Astlib_first_draft_parsetree.class_description -> Stable.V4_07.Class_description.t =
...
and class_type_declaration_to_ast : Astlib_first_draft_parsetree.class_type_declaration -> Stable.V4_07.Class_type_declaration.t =
  fun { pci_virt; pci_params; pci_name; pci_expr; pci_loc; pci_attributes } ->
    let pci_virt = virtual_flag_to_ast pci_virt in
    let pci_params = List.map ~f:(type_param_to_ast) pci_params in
    let pci_name = string_loc_to_ast pci_name in
...
and class_expr_to_ast : Astlib_first_draft_parsetree.class_expr -> Stable.V4_07.Class_expr.t =
...
and class_expr_desc_to_ast : Astlib_first_draft_parsetree.class_expr_desc -> Stable.V4_07.Class_expr_desc.t =
  function
  | Pcl_constr (a, b) ->
    let a = longident_loc_to_ast a in
    let b = List.map ~f:(core_type_to_ast) b in
...
and class_structure_to_ast : Astlib_first_draft_parsetree.class_structure -> Stable.V4_07.Class_structure.t =
...
and class_field_to_ast : Astlib_first_draft_parsetree.class_field -> Stable.V4_07.Class_field.t =
...
and class_field_desc_to_ast : Astlib_first_draft_parsetree.class_field_desc -> Stable.V4_07.Class_field_desc.t =
  function
  | Pcf_inherit (a, b, c) ->
    let a = override_flag_to_ast a in
...
and class_value_desc_to_ast : Astlib_first_draft_parsetree.class_value_desc -> Stable.V4_07.Class_value_desc.t =
  fun (a, b, c) ->
    let a = label_loc_to_ast a in
    let b = mutable_flag_to_ast b in
...
and class_method_desc_to_ast : Astlib_first_draft_parsetree.class_method_desc -> Stable.V4_07.Class_method_desc.t =
  fun (a, b, c) ->
    let a = label_loc_to_ast a in
    let b = private_flag_to_ast b in
    let c = class_field_kind_to_ast c in
...
and class_field_kind_to_ast : Astlib_first_draft_parsetree.class_field_kind -> Stable.V4_07.Class_field_kind.t =
  function
  | Cfk_virtual (a) ->
    let a = core_type_to_ast a in
...
and class_declaration_to_ast : Astlib_first_draft_parsetree.class_declaration -> Stable.V4_07.Class_declaration.t =
  fun { pci_virt; pci_params; pci_name; pci_expr; pci_loc; pci_attributes } ->
    let pci_virt = virtual_flag_to_ast pci_virt in
    let pci_params = List.map ~f:(type_param_to_ast) pci_params in
    let pci_name = string_loc_to_ast pci_name in
...
and module_type_to_ast : Astlib_first_draft_parsetree.module_type -> Stable.V4_07.Module_type.t =
...
and module_type_desc_to_ast : Astlib_first_draft_parsetree.module_type_desc -> Stable.V4_07.Module_type_desc.t =
  function
  | Pmty_ident (a) ->
    let a = longident_loc_to_ast a in
...
and signature_to_ast : Astlib_first_draft_parsetree.signature -> Stable.V4_07.Signature.t =
  fun a ->
    let a = List.map ~f:(signature_item_to_ast) a in
    Stable.V4_07.Signature.of_concrete (Signature { a })

and signature_item_to_ast : Astlib_first_draft_parsetree.signature_item -> Stable.V4_07.Signature_item.t =
...
and signature_item_desc_to_ast : Astlib_first_draft_parsetree.signature_item_desc -> Stable.V4_07.Signature_item_desc.t =
  function
  | Psig_value (a) ->
...
and module_declaration_to_ast : Astlib_first_draft_parsetree.module_declaration -> Stable.V4_07.Module_declaration.t =
...
and module_type_declaration_to_ast : Astlib_first_draft_parsetree.module_type_declaration -> Stable.V4_07.Module_type_declaration.t =
...
and open_description_to_ast : Astlib_first_draft_parsetree.open_description -> Stable.V4_07.Open_description.t =
...
and include_description_to_ast : Astlib_first_draft_parsetree.include_description -> Stable.V4_07.Include_description.t =
...
and include_declaration_to_ast : Astlib_first_draft_parsetree.include_declaration -> Stable.V4_07.Include_declaration.t =
  fun { pincl_mod; pincl_loc; pincl_attributes } ->
...
and with_constraint_to_ast : Astlib_first_draft_parsetree.with_constraint -> Stable.V4_07.With_constraint.t =
  function
  | Pwith_type (a, b) ->
    let a = longident_loc_to_ast a in
...
and module_expr_to_ast : Astlib_first_draft_parsetree.module_expr -> Stable.V4_07.Module_expr.t =
...
and module_expr_desc_to_ast : Astlib_first_draft_parsetree.module_expr_desc -> Stable.V4_07.Module_expr_desc.t =
  function
  | Pmod_ident (a) ->
    let a = longident_loc_to_ast a in
...
and structure_to_ast : Astlib_first_draft_parsetree.structure -> Stable.V4_07.Structure.t =
  fun a ->
    let a = List.map ~f:(structure_item_to_ast) a in
    Stable.V4_07.Structure.of_concrete (Structure { a })

and structure_item_to_ast : Astlib_first_draft_parsetree.structure_item -> Stable.V4_07.Structure_item.t =
...
and structure_item_desc_to_ast : Astlib_first_draft_parsetree.structure_item_desc -> Stable.V4_07.Structure_item_desc.t =
  function
  | Pstr_eval (a, b) ->
    let a = expression_to_ast a in
    let b = attributes_to_ast b in
...
and value_binding_to_ast : Astlib_first_draft_parsetree.value_binding -> Stable.V4_07.Value_binding.t =
...
and module_binding_to_ast : Astlib_first_draft_parsetree.module_binding -> Stable.V4_07.Module_binding.t =
...
and toplevel_phrase_to_ast : Astlib_first_draft_parsetree.toplevel_phrase -> Stable.V4_07.Toplevel_phrase.t =
  function
  | Ptop_def (a) ->
    let a = structure_to_ast a in
...
and directive_argument_to_ast : Astlib_first_draft_parsetree.directive_argument -> Stable.V4_07.Directive_argument.t =
  function
  | Pdir_none ->
