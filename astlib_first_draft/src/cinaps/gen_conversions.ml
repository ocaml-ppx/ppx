...
let rec substitute_ty ty ~arg : _ Astlib_first_draft_parsetree_types.ty =
  match (ty : _ Astlib_first_draft_parsetree_types.ty) with
  | (Bool | Char | String | Location | T _) as ty -> ty
  | A () -> arg
  | List ty -> List (substitute_ty ty ~arg)
  | Option ty -> Option (substitute_ty ty ~arg)

let substitute_rep rep ~arg : _ Astlib_first_draft_parsetree_types.rep =
  match (rep : _ Astlib_first_draft_parsetree_types.rep) with
...
let instantiate alist ~poly ~arg =
  match (List.assoc_opt poly alist : Astlib_first_draft_parsetree_types.decl option) with
  | Some (Poly rep) -> substitute_rep rep ~arg
  | _ -> assert false

let module_name ~name =
  Printf.sprintf "Stable.%s.%s"
    Astlib_first_draft_parsetree_types.version
...
let tree_type ~name = Printf.sprintf "Astlib_first_draft_parsetree.%s" name

let print_conversions_mli () =
  List.iter Astlib_first_draft_parsetree_types.t ~f:(fun (name, decl) ->
    match (decl : Astlib_first_draft_parsetree_types.decl) with
    | Poly _ -> ()
    | Inst _ | Mono _ ->
      Print.newline ();
...
let rec to_ast ty =
  match (ty : Astlib_first_draft_parsetree_types.nothing Astlib_first_draft_parsetree_types.ty) with
...
let print_conversions_ml () =
  let alist = Astlib_first_draft_parsetree_types.t in
  let reps =
