...
let rec string_of_ty (ty : _ Astlib_first_draft_parsetree_types.ty) =
  match ty with
  | Bool -> "bool"
  | Char -> "char"
  | String -> "string"
...
let print_parsetree_rep (rep : _ Astlib_first_draft_parsetree_types.rep) =
  match rep with
...
let print_parsetree_decl (decl : Astlib_first_draft_parsetree_types.decl) =
  match decl with
...
let arg_of_decl (decl : Astlib_first_draft_parsetree_types.decl) =
  match decl with
  | Poly _ -> "'a "
  | Mono _ | Inst _ -> ""

let equation_of_rep (rep : _ Astlib_first_draft_parsetree_types.rep) ~arg =
  match rep with
  | Alias _ | Tuple _ -> ""
  | Record { source; _ } | Variant { source; _ } ->
    Printf.sprintf " %s%s =" arg source

let equation_of_decl (decl : Astlib_first_draft_parsetree_types.decl) =
